...
from modules import (
    initialize_system, get_config, get_db, get_system_info,
    Work, Release, WorkType, ReleaseType, DataSource,
    get_anilist_collector, get_rss_collector
)
...
def demo_data_models():
    ...
    print(f"✓ Release model: {release1.release_type.value} {release1.number} on {release1.platform}")
...
def demo_query_operations(db):
    ...
    if unnotified:
        ...
        print(f"  - Sample: {sample_release['title']} - {sample_release['release_type']} {sample_release.get('number', 'N/A')}")
...
def demo_data_collection_concepts(config):
    ...
    try:
        ...
        print(f"  - Rate limit: {config.get('apis.anilist.rate_limit.requests_per_minute')}/min")
...
def demo_work_management_workflow(db):
    ...
    for episode in episodes:
        ...
        if release_id:
            print(f"  - Episode {episode['number']}: {episode['date']} on {episode['platform']}")
...
def main():
    ...
    try:
        ...
        print("\n=== Final System Status ===")
        stats = db.get_work_stats()
        print(f"Total works in database: {sum(v for k, v in stats.items() if k.endswith('_works'))}")
