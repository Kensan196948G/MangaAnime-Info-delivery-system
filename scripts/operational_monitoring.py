...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/mnt/Linux-ExHDD/MangaAnime-Info-delivery-system/logs/operational_monitoring.log'),
...
class OperationalMonitor:
    ...
    def check_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_status["components"]["cpu"] = "healthy" if cpu_usage < self.alert_thresholds["cpu_usage_max"] else "warning"
            ...
            health_status["components"]["memory"] = "healthy" if memory.percent < self.alert_thresholds["memory_usage_max"] else "warning"
            ...
            health_status["components"]["disk"] = "healthy" if disk_percent < self.alert_thresholds["disk_usage_max"] else "critical"
            ...
            if db_status["response_time"] >= self.alert_thresholds["db_response_time_max"]:
                health_status["alerts"].append(f"データベース応答が遅い: {db_status['response_time']:.1f}ms")
    ...
    def check_database_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "healthy" if response_time < self.alert_thresholds["db_response_time_max"] else "warning",
    ...
    def check_external_apis(self) -> Dict[str, Any]:
        ...
        overall_status = "healthy" if healthy_count == len(apis_to_check) else "warning" if healthy_count > 0 else "error"
    ...
    def check_scheduled_tasks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if cron_result.returncode == 0:
                cron_lines = [line.strip() for line in cron_result.stdout.split('\n') if line.strip()]
                anime_tasks = [line for line in cron_lines if 'release_notifier' in line or 'anime' in line.lower()]
    ...
    def generate_daily_report(self) -> Dict[str, Any]:
        ...
        report["daily_summary"] = {
            ...
            "healthy_components": sum(1 for status in latest_health["components"].values() if status == "healthy"),
...
def run_immediate_health_check():
    ...
    print("\n📋 日次サマリー:")
    print(f"  健全なコンポーネント: {daily_report['daily_summary']['healthy_components']}/{daily_report['daily_summary']['total_components']}")
    ...
    # レポート保存
    report_path = "/mnt/Linux-ExHDD/MangaAnime-Info-delivery-system/logs/health_check_report.json"
