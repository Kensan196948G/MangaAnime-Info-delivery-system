...
class RepairLoopManager:
    ...
    def can_attempt_repair(self):
        ...
        if state['in_cooldown'] and state['cycle_end_time']:
            ...
            if datetime.now() < cooldown_end:
                ...
                print(f"⏸️  Cooldown active. Remaining: {int(remaining / 60)} minutes {int(remaining % 60)} seconds")
                return False, state
            else:
                # クールダウン終了、次のサイクルへ
        ...
        if state['attempts_in_cycle'] >= self.max_attempts_per_cycle:
            ...
            self.save_state(state)
            print(f"💤 Cycle {state['current_cycle']} complete. Entering {self.cooldown_minutes} minute cooldown...")
            return False, state
    ...
    def record_attempt(self, success=False, error_details=None):
        ...
        print(f"📊 Cycle {state['current_cycle']}, Attempt {state['attempts_in_cycle']}/{self.max_attempts_per_cycle']}")
    ...
    def get_status_summary(self):
        ...
        if state['repair_history']:
            ...
            summary['success_rate'] = f"{(successful / len(state['repair_history']) * 100):.1f}%"
...
def main():
    ...
    parser.add_argument('action', choices=['check', 'attempt', 'status', 'reset'],
                      help='Action to perform')
    parser.add_argument('--success', action='store_true',
                      help='Mark attempt as successful')
    parser.add_argument('--error', type=str,
                      help='Error details for failed attempt')
