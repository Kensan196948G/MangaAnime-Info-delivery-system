...
class DatabaseAnalyzer:
    ...
    def analyze(self) -> None:
        """データベース全体を分析"""
        logger.info("=" * 80)
        logger.info("📊 データベース分析レポート")
        logger.info("=" * 80)
        logger.info(f"📅 分析日時: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        logger.info(f"📁 データベース: {self.db_path}")
        logger.info()
    ...
    def _analyze_indexes(self) -> None:
        ...
        if indexes:
            for name, table, sql in indexes:
                logger.info(f"\n📇 {name}")
                logger.info(f"   テーブル: {table}")
                logger.info(f"   定義: {sql}")
        else:
            logger.info("⚠️  カスタムインデックスが見つかりません")
            logger.info("💡 推奨: パフォーマンス向上のためインデックス追加を検討してください")

    def _analyze_foreign_keys(self) -> None:
        """外部キー制約分析"""
        logger.info("\n" + "=" * 80)
        logger.info("🔗 外部キー制約分析")
        logger.info("-" * 80)
        ...
        for table_name in tables:
            ...
            if fks:
                ...
                for fk in fks:
                    ...
                    logger.info(f"  - {from_col} → {table}({to_col})")
                    logger.info(f"    ON DELETE: {on_delete}, ON UPDATE: {on_update}")

        if not foreign_keys_found:
            logger.info("⚠️  外部キー制約が見つかりません")
            logger.info("💡 推奨: データ整合性のため外部キー制約の追加を検討してください")

    def _analyze_database_size(self) -> None:
        """データベースサイズ分析"""
        logger.info("\n" + "=" * 80)
        logger.info("💾 データベースサイズ")
        logger.info("-" * 80)
        ...
        logger.info(f"ファイルサイズ: {db_size:,} bytes ({db_size_mb:.2f} MB)")
        ...
        logger.info(f"ページサイズ: {page_size:,} bytes")
        ...
        if freelist > 0:
            ...
            logger.info(f"⚠️  未使用ページ: {freelist:,} ({waste_mb:.2f} MB)")
            logger.info("💡 推奨: VACUUM コマンドでデータベースを最適化してください")
    ...
    def print_recommendations(self) -> None:
        ...
        if recommendations:
            for i, rec in enumerate(recommendations, 1):
                ...
                if 'sql_file' in rec:
                    logger.info(f"   📁 {rec['sql_file']}")
                if 'command' in rec:
                    logger.info(f"   💻 {rec['command']}")
                if 'details' in rec:
                    logger.info(f"   ℹ️  {rec['details']}")
        else:
            logger.info("\n✅ 特に問題は見つかりませんでした")
...
def main():
    ...
    try:
        ...
        logger.info(f"❌ エラー: {e}", file=sys.stderr)
        sys.exit(1)
    except Exception as e:
        logger.info(f"❌ 予期しないエラー: {e}", file=sys.stderr)
        import traceback
        traceback.print_exc()
        sys.exit(1)
