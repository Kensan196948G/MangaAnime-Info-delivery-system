...
class RepairAnalytics:
    ...
    def populate_data_if_needed(self):
        ...
        try:
            ...
            if count == 0:
                ...
                sample_repairs = [
                    ('workflow_fix', 'success', 45.2, None, 'auto-repair.yml', 'abc123', 0, 0.3, 4500),
                    ('dependency_update', 'failure', 120.5, 'Package not found', 'dependencies.yml', 'def456', 1, 0.8, 4450),
                    ('syntax_error', 'success', 30.1, None, 'lint.yml', 'ghi789', 0, 0.2, 4400),
                    ('api_timeout', 'failure', 180.0, 'Request timeout', 'api-test.yml', 'jkl012', 2, 0.9, 4300),
                    ('security_scan', 'success', 90.3, None, 'security.yml', 'mno345', 0, 0.4, 4250),
                ]
                ...
                conn.commit()
                print(f"{Colors.YELLOW}Populated database with sample data for analysis{Colors.END}")
            
            conn.close()
        except Exception as e:
            print(f"{Colors.YELLOW}Warning: Could not populate sample data: {e}{Colors.END}")
            ...
            for row in cursor.fetchall():
                ...
                if errors:
                    ...
                    if error_messages:
                        ...
                        error_list = [f"{error} ({count}x)" for error, count in error_counter.most_common(3)]
                ...
                time_pattern = f"Peak failures at {time_result[0]}:00" if time_result else "No clear pattern"
                
                # Determine severity
                severity = "high" if success_rate < 50 else "medium" if success_rate < 80 else "low"
                
                # Generate recommendations
                recommendations = self.generate_pattern_recommendations(repair_type, success_rate, avg_duration or 0)
    ...
    def generate_pattern_recommendations(self, repair_type: str, success_rate: float, avg_duration: float) -> List[str]:
        """Generate recommendations based on failure patterns"""
        recommendations = []
        
        if success_rate < 50:
            recommendations.append(f"Critical: {repair_type} has very low success rate - review workflow logic")
            recommendations.append("Consider adding more robust error handling and retry mechanisms")
        elif success_rate < 80:
            recommendations.append(f"Moderate: {repair_type} needs improvement - add better validation")
        
        if avg_duration > 120:  # More than 2 minutes
            recommendations.append("Optimize for faster execution - consider parallel processing")
    ...
    def calculate_performance_metrics(self) -> PerformanceMetrics:
        ...
        try:
            ...
            system_availability = max(0, 100 - (failures_24h * 2))  # Assume each failure causes ~2% downtime
    ...
    def detect_anomalies(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            features = ['duration', 'retry_count', 'system_load', 'github_api_remaining', 'hour']
            ...
            for i, is_anomaly in enumerate(anomalies):
                if is_anomaly == -1:  # Anomaly detected
                    ...
                    anomaly_records.append({
                        ...
                        'anomaly_score': iso_forest.decision_function(X_scaled[i:i+1])[0]
                    })
    ...
    def generate_optimization_recommendations(self) -> List[OptimizationRecommendation]:
        ...
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            cursor.execute("SELECT AVG(github_api_remaining) FROM repair_history WHERE github_api_remaining > 0")
    ...
    def generate_trend_analysis(self, days: int = 30):
        ...
        try:
            ...
            if df.empty:
                print(f"{Colors.YELLOW}No data available for trend analysis{Colors.END}")
                return
            ...
            if len(df) > 1:
                ...
                trend_direction = "↗️ improving" if success_trend > 0 else "↘️ declining" if success_trend < 0 else "→ stable"
                print(f"   Success Rate Trend: {Colors.CYAN}{trend_direction} ({success_trend:.2f}%/day){Colors.END}")
                
                # Duration trend
                duration_trend = np.polyfit(range(len(df)), df['avg_duration'], 1)[0]
                duration_direction = "↗️ increasing" if duration_trend > 0 else "↘️ decreasing" if duration_trend < 0 else "→ stable"
                print(f"   Duration Trend:     {Colors.CYAN}{duration_direction} ({duration_trend:.2f}s/day){Colors.END}")
                
                # Volume trend
                volume_trend = np.polyfit(range(len(df)), df['total_repairs'], 1)[0]
                volume_direction = "↗️ increasing" if volume_trend > 0 else "↘️ decreasing" if volume_trend < 0 else "→ stable"
                print(f"   Repair Volume:      {Colors.CYAN}{volume_direction} ({volume_trend:.2f}/day){Colors.END}")
            
            # Statistical summary
            print(f"\n{Colors.YELLOW}📊 STATISTICAL SUMMARY{Colors.END}")
            print(f"   Average Success Rate: {Colors.GREEN}{df['success_rate'].mean():.1f}%{Colors.END}")
            print(f"   Success Rate StdDev:  {Colors.CYAN}{df['success_rate'].std():.1f}%{Colors.END}")
            print(f"   Average Duration:     {Colors.GREEN}{df['avg_duration'].mean():.1f}s{Colors.END}")
            print(f"   Total Repairs:        {Colors.GREEN}{df['total_repairs'].sum()}{Colors.END}")
    ...
    def create_visualizations(self, output_dir: str = "analytics_output"):
        ...
        try:
            ...
            if not df_time.empty:
                ...
                df_time['success_rate'] = (df_time['successful'] / df_time['total']) * 100
                ...
                if not df_patterns.empty:
                    plt.subplot(2, 2, 3)
                    plt.pie(df_patterns['count'], labels=df_patterns['repair_type'], autopct='%1.1f%%')
                ...
                if not df_duration.empty:
                    plt.subplot(2, 2, 4)
                    plt.hist(df_duration['duration'], bins=20, alpha=0.7, color='orange')
                    plt.title('Repair Duration Distribution')
                    plt.xlabel('Duration (seconds)')
                    plt.ylabel('Frequency')
                
                plt.tight_layout()
                plt.savefig(f"{output_dir}/repair_analytics_overview.png", dpi=300, bbox_inches='tight')
                print(f"{Colors.GREEN}Saved overview visualization to {output_dir}/repair_analytics_overview.png{Colors.END}")
    ...
    def export_analysis_report(self, output_file: str):
        ...
        try:
            ...
            print(f"{Colors.GREEN}Analysis report exported to: {output_file}{Colors.END}")
    ...
    def display_analytics_dashboard(self):
        ...
        print(f"{Colors.BOLD}{Colors.CYAN}        AUTO-REPAIR ANALYTICS DASHBOARD        {Colors.END}")
        print(f"{Colors.BLUE}{'='*80}{Colors.END}")
        ...
        print(f"   Overall Success Rate:    {self.colorize_metric(metrics.overall_success_rate, 90, 95)}%")
        print(f"   Avg Repair Duration:     {Colors.CYAN}{metrics.avg_repair_duration:.1f}s{Colors.END}")
        print(f"   Repairs per Day:         {Colors.CYAN}{metrics.repairs_per_day:.1f}{Colors.END}")
        print(f"   Failure Recovery Rate:   {self.colorize_metric(metrics.failure_recovery_rate, 70, 85)}%")
        print(f"   System Availability:     {self.colorize_metric(metrics.system_availability, 95, 99)}%")
        print(f"   MTTR (Mean Time To Repair): {Colors.CYAN}{metrics.mttr:.1f}s{Colors.END}")
        print(f"   MTBF (Mean Time Between Failures): {Colors.CYAN}{metrics.mtbf:.1f}h{Colors.END}")
        print()
        ...
        if patterns:
            for pattern in patterns[:3]:  # Show top 3 patterns
                severity_color = Colors.RED if pattern.severity == "high" else Colors.YELLOW if pattern.severity == "medium" else Colors.GREEN
                print(f"   {severity_color}[{pattern.severity.upper()}]{Colors.END} {pattern.pattern_type}")
                print(f"     └─ Frequency: {pattern.frequency}, Success Rate: {pattern.success_rate:.1f}%")
        else:
            print(f"   {Colors.GREEN}No significant failure patterns detected{Colors.END}")
        print()
        ...
        if recommendations:
            for rec in recommendations[:3]:  # Show top 3 recommendations
                priority_color = Colors.RED if rec.priority == "high" else Colors.YELLOW if rec.priority == "medium" else Colors.GREEN
                print(f"   {priority_color}[{rec.priority.upper()}]{Colors.END} {rec.category}: {rec.description}")
                print(f"     └─ Expected: {rec.expected_impact} (Improvement: {rec.estimated_improvement:.1f}%)")
        else:
            print(f"   {Colors.GREEN}System is well optimized - no critical recommendations{Colors.END}")
        print()
        ...
        if anomalies:
            print(f"   {Colors.RED}Detected {len(anomalies)} anomalous repair operations{Colors.END}")
            for anomaly in anomalies[:2]:  # Show top 2 anomalies
                print(f"     └─ Duration: {anomaly['duration']:.1f}s, Retries: {anomaly['retry_count']}")
        else:
            print(f"   {Colors.GREEN}No anomalies detected in recent operations{Colors.END}")
        print()
...
def main():
    ...
    parser.add_argument('--dashboard', '-d', action='store_true', help='Show analytics dashboard')
    parser.add_argument('--patterns', '-p', action='store_true', help='Analyze failure patterns')
    parser.add_argument('--recommendations', '-r', action='store_true', help='Generate optimization recommendations')
    parser.add_argument('--trends', '-t', type=int, metavar='DAYS', default=30, help='Analyze trends for N days')
    parser.add_argument('--anomalies', '-a', action='store_true', help='Detect anomalies')
    ...
    parser.add_argument('--output-dir', default='analytics_output', help='Output directory for files')
