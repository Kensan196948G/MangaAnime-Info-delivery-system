...
class SecuritySetup:
    ...
    def _setup_environment_variables(self):
        ...
        print("\n🔐 Would you like to set up OAuth2 client credentials now? (y/n): ", end="")
        ...
        if setup_oauth:
            ...
            gmail_client_secret = getpass.getpass("Enter Gmail OAuth2 Client Secret: ").strip()
            if gmail_client_secret:
                env_vars['GMAIL_CLIENT_SECRET'] = gmail_client_secret
            
            calendar_client_id = input("Enter Calendar OAuth2 Client ID (or press Enter to use Gmail ID): ").strip()
            ...
            if not calendar_client_id:
                ...
                calendar_client_secret = getpass.getpass("Enter Calendar OAuth2 Client Secret: ").strip()
    ...
    def _write_env_file(self, env_vars: Dict[str, str]):
        ...
        sections = {
            "System Settings": ['MANGA_ANIME_ENVIRONMENT', 'MANGA_ANIME_LOG_LEVEL', 'MANGA_ANIME_LOG_PATH'],
            "Database": ['MANGA_ANIME_DB_PATH'],
            "Gmail Configuration": ['MANGA_ANIME_GMAIL_FROM', 'MANGA_ANIME_GMAIL_TO', 'GMAIL_CLIENT_ID', 'GMAIL_CLIENT_SECRET'],
            "Calendar Configuration": ['MANGA_ANIME_CALENDAR_ID', 'CALENDAR_CLIENT_ID', 'CALENDAR_CLIENT_SECRET'],
            "Google API Files": ['MANGA_ANIME_CREDENTIALS_FILE', 'MANGA_ANIME_TOKEN_FILE'],
            "Security Keys": ['MANGA_ANIME_SECRET_KEY', 'MANGA_ANIME_ENCRYPTION_KEY', 'MANGA_ANIME_SALT', 'MANGA_ANIME_MASTER_PASSWORD']
        }
    ...
    def _validate_security_setup(self):
        ...
        if stat.filemode(env_stat.st_mode) != '-rw-------':
            logger.warning(f"Environment file permissions may be too permissive: {stat.filemode(env_stat.st_mode)}")
