...
class TestRunner:
    ...
    def _build_pytest_command(self, test_type: str, **kwargs) -> List[str]:
        ...
        if 'html' in output_format:
            cmd.extend([
                '--html', str(self.reports_dir / f'{test_type}_report_{timestamp}.html'),
                '--self-contained-html'
            ])
        
        if 'xml' in output_format or 'junit' in output_format:
            cmd.extend([
                '--junitxml', str(self.reports_dir / f'{test_type}_junit_{timestamp}.xml')
            ])
        
        if 'json' in output_format:
            cmd.extend([
                '--json-report',
                '--json-report-file', str(self.reports_dir / f'{test_type}_results_{timestamp}.json')
            ])
        ...
        if test_type == 'performance':
            cmd.extend([
                '--benchmark-json', str(self.reports_dir / f'benchmark_{timestamp}.json'),
                '--benchmark-histogram', str(self.reports_dir / f'benchmark_histogram_{timestamp}')
            ])
...
{test_results['stdout']}
    ...
    def _generate_performance_analysis(self, test_results: Dict[str, Any], timestamp: str):
    ...
    def _generate_quality_metrics_report(self, test_results: Dict[str, Any], timestamp: str):
    ...
    def _calculate_quality_score(self, test_results: Dict[str, Any]) -> float:
        ...
        if 'tests_failed' in test_results and test_results['tests_failed'] > 0:
            total_tests = (test_results.get('tests_passed', 0) + 
        ...
        execution_time = test_results['execution_time']
        max_time = self.performance_thresholds.get(f"{test_results['test_type']}_tests_max_time", 300)
    ...
    def _generate_recommendations(self, test_results: Dict[str, Any]) -> List[str]:
        ...
        max_time = self.performance_thresholds.get(f"{test_results['test_type']}_tests_max_time", 300)
        if execution_time > max_time:
            recommendations.append(f"Optimize test execution time (current: {execution_time}s, target: <{max_time}s)")
...
def main():
    ...
    parser.add_argument('--output-format', choices=['term', 'html', 'xml', 'json'], default='html', help='Output format')
    parser.add_argument('--clean-reports', type=int, help='Clean reports older than N days')
