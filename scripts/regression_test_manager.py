...
class RegressionTestManager:
    ...
    def create_baseline(self, version: str = "auto") -> RegressionBaseline:
        ...
        baseline_file = self.baselines_dir / f"baseline_{baseline.timestamp.replace(':', '-')}.json"
    ...
    def compare_with_baseline(self, baseline_id: Optional[int] = None) -> RegressionComparison:
    ...
    def _get_baseline(self, baseline_id: Optional[int] = None) -> Optional[RegressionBaseline]:
    ...
    def _perform_comparison(self, current: RegressionBaseline, 
                           previous: RegressionBaseline) -> RegressionComparison:
        ...
        overall_score = self._calculate_overall_score(
            test_regressions, performance_regressions, coverage_regressions, improvements
        )
        
        # Generate recommendation
        recommendation = self._generate_recommendation(
            test_regressions, performance_regressions, coverage_regressions, overall_score
        )
    ...
    def _compare_test_results(self, current: RegressionBaseline, 
                             previous: RegressionBaseline) -> List[Dict[str, Any]]:
    ...
    def _compare_performance_metrics(self, current: RegressionBaseline,
                                   previous: RegressionBaseline) -> List[Dict[str, Any]]:
    ...
    def _compare_coverage_data(self, current: RegressionBaseline,
                              previous: RegressionBaseline) -> List[Dict[str, Any]]:
        """Compare coverage data for regressions"""
        regressions = []
        
        current_coverage = current.coverage_data.get('totals', {}).get('percent_covered', 0)
        previous_coverage = previous.coverage_data.get('totals', {}).get('percent_covered', 0)
    ...
    def _identify_improvements(self, current: RegressionBaseline,
                              previous: RegressionBaseline) -> List[Dict[str, Any]]:
        ...
        if current_summary.get('total', 0) > previous_summary.get('total', 0):
            improvements.append({
                ...
                'improvement': current_summary.get('total', 0) - previous_summary.get('total', 0)
            })
    ...
    def _calculate_overall_score(self, test_regressions: List, performance_regressions: List,
                               coverage_regressions: List, improvements: List) -> float:
    ...
    def _generate_recommendation(self, test_regressions: List, performance_regressions: List,
                               coverage_regressions: List, overall_score: float) -> str:
    ...
    def _save_comparison_report(self, comparison: RegressionComparison):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """, (
                ...
                len(comparison.test_regressions + comparison.performance_regressions + comparison.coverage_regressions),
        ...
        # Save to file
        report_file = self.baselines_dir / f"regression_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            'recent_reports': [
                {
                    'timestamp': r[0],
                    'score': r[1],
                    'regression_count': r[2]
                } for r in reports
            ],
            'recommendations': self._generate_summary_recommendations(avg_score, reports)
        }
    
    def _generate_summary_recommendations(self, avg_score: float, reports: List) -> List[str]:
        """Generate recommendations based on summary statistics"""
        recommendations = []
        
        if avg_score < 70:
            recommendations.append("Focus on fixing recurring regression issues")
            recommendations.append("Implement additional regression prevention measures")
        
        if len(reports) < 3:
            recommendations.append("Create more regression baselines for better tracking")
        
        if reports:
            recent_scores = [r[1] for r in reports[:3] if r[1] is not None]
            if len(recent_scores) >= 2 and all(recent_scores[i] < recent_scores[i+1] for i in range(len(recent_scores)-1)):
                recommendations.append("Regression scores are trending downward - investigate root causes")
...
def main():
    ...
    parser.add_argument('--create-baseline', action='store_true', help='Create new baseline')
    parser.add_argument('--version', default='auto', help='Version for baseline')
    parser.add_argument('--compare', action='store_true', help='Compare with latest baseline')
    parser.add_argument('--baseline-id', type=int, help='Specific baseline ID to compare with')
