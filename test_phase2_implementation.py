...
from modules.manga_rss import MangaRSSCollector, BookWalkerRSSCollector, DAnimeRSSCollector
from modules.data_normalizer import (
    TitleNormalizer, WorkMatcher, DataQualityAnalyzer, DataIntegrator,
    normalize_title, find_duplicate_works, integrate_work_data, analyze_data_quality
)
...
class Phase2TestSuite:
    ...
    def test_anilist_enhancements(self):
        ...
        expected_keys = ['request_count', 'error_count', 'error_rate', 'average_response_time']
    ...
    def test_rss_enhancements(self):
        ...
        assert bookwalker_collector is not None, "BookWalker collector initialization failed"
        assert hasattr(bookwalker_collector, 'bookwalker_base_url'), "Missing BookWalker specific config"
    ...
    def test_data_normalization(self):
        ...
        assert normalizer._detect_language(ja_title) == "ja", "Japanese detection failed"
    ...
    def test_data_integration(self):
        ...
        assert len(integrated) <= len(test_works), "Integration should not increase work count"
    ...
    def test_collection_api(self):
        ...
        expected_keys = ['total_jobs', 'successful_jobs', 'failed_jobs', 'average_duration']
        for key in expected_keys:
        ...
        try:
            ...
            assert hasattr(quality_report, 'total_works'), "Quality report missing total_works"
    ...
    def test_performance_quality(self):
        ...
        expected_stats = [
            'request_count', 'error_count', 'error_rate', 'average_response_time',
            'circuit_breaker_state', 'circuit_failure_count', 'rate_limit_queue_size'
        ]
        ...
        assert duration < 5.0, f"Normalization too slow: {duration}s for {len(test_titles)} titles"
        print(f"    ✓ Title normalization performance: {len(test_titles)} titles in {duration:.2f}s")
    ...
    def _print_test_summary(self):
        ...
        if passed_count == total_count:
            ...
            print("The enhanced backend information collection functionality is ready for use.")
        else:
