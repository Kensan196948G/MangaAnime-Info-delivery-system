...
def integrate_auth_py():
    ...
    if not auth_file.exists():
        logger.info(f"❌ ファイルが見つかりません: {auth_file}")
        return False
    ...
    if 'def get_all_users(self)' in content:
        logger.info("⚠️  auth.py: すでに統合済みです")
        return True
...
'''
    ...
    if new_content == content:
        logger.info("❌ auth.py: パターンマッチに失敗しました")
        return False

    auth_file.write_text(new_content, encoding='utf-8')
    logger.info("✅ auth.py: UserStoreメソッドを追加しました")
    return True
...
def integrate_web_app_py():
    ...
    if not web_app_file.exists():
        logger.info(f"❌ ファイルが見つかりません: {web_app_file}")
        return False
    ...
    if 'from app.routes.users import users_bp' in content:
        logger.info("⚠️  web_app.py: すでに統合済みです")
        return True
    ...
    logger.info("✅ web_app.py: users_bpを登録しました")
    return True
...
def integrate_base_html():
    ...
    if not base_file.exists():
        logger.info(f"❌ ファイルが見つかりません: {base_file}")
        return False
    ...
    if 'users.user_list' in content:
        logger.info("⚠️  base.html: すでに統合済みです")
        return True
...
'''
    ...
    if new_content == content:
        logger.info("❌ base.html: パターンマッチに失敗しました")
        return False

    base_file.write_text(new_content, encoding='utf-8')
    logger.info("✅ base.html: ナビゲーションリンクを追加しました")
    return True
...
logger = logging.getLogger(__name__)
    ...
    for file_path, name in checks:
        if file_path.exists():
            ...
            logger.info(f"❌ {name} (ファイルが存在しません)")
            all_ok = False
...
def main():
    """メイン処理"""
    logger.info("=" * 60)
    logger.info("ユーザー管理機能 自動統合スクリプト")
    logger.info("=" * 60)
    logger.info()
    ...
    logger.info("1. auth.pyへのUserStoreメソッド追加...")
    ...
    # 結果サマリー
    logger.info("\n" + "=" * 60)
    logger.info("統合完了サマリー")
    logger.info("=" * 60)

    if all(results):
        logger.info("✅ すべての統合が正常に完了しました")
        logger.info()
        logger.info("次のステップ:")
        logger.info("  1. アプリケーションを起動: python app/web_app.py")
        logger.info("  2. 管理者でログイン")
        logger.info("  3. ナビゲーションバーの「ユーザー管理」をクリック")
        logger.info()
        logger.info("詳細な使用方法: docs/USER_MANAGEMENT_INTEGRATION.md")
        return 0
    else:
        logger.info("❌ 一部の統合に失敗しました")
        logger.info()
        logger.info("トラブルシューティング:")
        logger.info("  1. バックアップファイル(.bak)から復元")
        logger.info("  2. 手動統合ガイドを参照: docs/USER_MANAGEMENT_INTEGRATION.md")
        return 1
