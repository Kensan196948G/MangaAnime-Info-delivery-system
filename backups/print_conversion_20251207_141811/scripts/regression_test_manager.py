...
class RegressionTestManager:
    ...
    def create_baseline(self, version: str = "auto") -> RegressionBaseline:
        ...
        logger.info("📊 Creating regression baseline...")
        ...
        logger.info(f"✅ Baseline created: {baseline.timestamp}")
        return baseline
    ...
    def _collect_test_results(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"⚠️  Could not collect test results: {e}")
    ...
    def _collect_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"⚠️  Could not collect performance metrics: {e}")
    ...
    def _collect_coverage_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"⚠️  Could not collect coverage data: {e}")
    ...
    def compare_with_baseline(
        ...
        if not previous_baseline:
            logger.info("⚠️  No baseline found for comparison")
            return None
    ...
    def _save_comparison_report(self, comparison: RegressionComparison):
        ...
        logger.info(f"📄 Regression report saved: {report_file}")
...
logger = logging.getLogger(__name__)
    ...
    try:
        if args.create_baseline:
            ...
            if comparison:
                logger.info("\n" + "=" * 60)
                logger.info("🔍 REGRESSION COMPARISON RESULTS")
                logger.info("=" * 60)
                logger.info(f"Overall Score: {comparison.overall_score:.1f}/100")
            ...
            logger.info("\n" + "=" * 60)
            logger.info("📊 REGRESSION TESTING SUMMARY")
            logger.info("=" * 60)
            logger.info(f"Total Baselines: {summary['baseline_count']}")
            logger.info(f"Recent Reports: {summary['recent_reports_count']}")
            ...
            logger.info("Use --create-baseline, --compare, or --summary")
