...
WORKFLOW_PERMISSIONS = {
    ...
    "system-health-fix": {"contents": "read", "issues": "write", "actions": "read"},
    "ci": {
        "contents": "read",
        "issues": "write",
        "pull-requests": "write",
...
def fix_workflow_permissions(workflow_path):
    ...
    if re.search(r"^permissions:", content, re.MULTILINE):
        logger.info(f"✅ {filename}: Already has permissions")
        return False
    ...
    try:
        ...
        logger.info(f"❌ {filename}: YAML parse error: {e}")
        return False
    ...
    logger.info(f"✅ {filename}: Added permissions")
    return True
...
logger = logging.getLogger(__name__)
    ...
    if not workflows_dir.exists():
        logger.info(f"❌ Directory not found: {workflows_dir}")
        return 1
    ...
    for workflow_file in workflows_dir.glob("*.yml"):
        with open(workflow_file, "r") as f:
            ...
            if "permissions:" in content:
                logger.info(f"  ✅ {workflow_file.name}: Has permissions")
            else:
                logger.info(f"  ❌ {workflow_file.name}: Missing permissions")
