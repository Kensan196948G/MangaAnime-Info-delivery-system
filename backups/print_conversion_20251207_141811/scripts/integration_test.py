...
class IntegrationTestSuite:
    ...
    def create_test_config(self):
        ...
        test_config = {
            ...
            "apis": {
                "anilist": {"url": "https://graphql.anilist.co", "rate_limit": 90},
                "gmail": {
                    "enabled": False,
            ...
            "notification": {
                "email": {"enabled": False, "recipient": "test@example.com"},
                "calendar": {"enabled": False, "calendar_id": "primary"},
            },
            "filtering": {"ng_keywords": ["エロ", "R18", "成人向け", "BL", "百合"]},
        }
    ...
    async def test_anilist_api_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 平均応答時間が5秒以内なら合格
            avg_response_time = sum(test_result["response_times"]) / len(
    ...
    def test_database_operations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            start_time = time.time()
            cursor.execute(
                "UPDATE works SET title = ? WHERE id = ?", ("更新テスト作品", work_id)
            )
            conn.commit()
    ...
    def test_notification_system(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_result["email_test"] = {
                ...
                "ready_for_production": credentials_path.exists()
    ...
    def test_filtering_logic(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if passed_tests == len(test_cases):
                ...
                test_result["status"] = "failed"
                test_result["errors"].append(
                    f"フィルタリングテスト {passed_tests}/{len(test_cases)} 通過"
                )
                ...
                try:
                    with open(app_py, "r", encoding="utf-8") as f:
                        ...
                        if "from flask import" in content and "app = Flask" in content:
                            test_result["file_checks"]["flask_structure"] = {
                                "valid": True
                            }
                        else:
                            test_result["file_checks"]["flask_structure"] = {
    ...
    async def run_full_integration_test(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_results["anilist_api"] = (
                async_results[0]
                if not isinstance(async_results[0], Exception)
                else {"status": "failed", "error": str(async_results[0])}
            )
    ...
    def compile_test_results(self, test_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        warning_tests = sum(
            1 for result in test_results.values() if result.get("status") == "warning"
        )
        failed_tests = sum(
        ...
        # 全体スコア計算
        overall_score = (
            (passed_tests * 100 + warning_tests * 50) / (total_tests * 100) * 100
            if total_tests > 0
            else 0
        )
        ...
        for test_name, result in test_results.items():
            if result.get("status") == "failed":
                recommendations.append(
                    f"❌ {test_name}: 修正が必要です - {', '.join(result.get('errors', []))}"
                )
            elif result.get("status") == "warning":
                recommendations.append(
                    f"⚠️ {test_name}: 改善を推奨します - {', '.join(result.get('errors', []))}"
                )
...
async def main():
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info(f"📊 全体ステータス: {results['overall_status'].upper()}")
        logger.info(f"🎯 全体スコア: {results['overall_score']:.1f}/100")
        logger.info(f"✅ 成功: {results['test_summary']['passed']}")
        logger.info(f"⚠️ 警告: {results['test_summary']['warning']}")
        logger.info(f"❌ 失敗: {results['test_summary']['failed']}")
        logger.info(f"⏱️ 実行時間: {results['execution_time']:.2f}秒")
