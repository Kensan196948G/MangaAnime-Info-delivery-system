...
def run_immediate_health_check():
    ...
    logger.info("🔍 システムヘルスチェック実行中...")
    health_status = monitor.check_system_health()

    # 結果表示
    logger.info("\n" + "=" * 60)
    logger.info("🏥 システムヘルスチェック結果")
    logger.info("=" * 60)
    logger.info(f"📊 総合ステータス: {health_status['overall_status'].upper()}")
    logger.info(f"⏰ チェック時刻: {health_status['timestamp']}")
    ...
    if "disk_usage" in metrics:
        logger.info(f"  💾 ディスク使用率: {metrics['disk_usage']:.1f}%")
    ...
    logger.info(f"  システム稼働時間: {daily_report['system_uptime']}")
