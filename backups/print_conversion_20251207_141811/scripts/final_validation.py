...
try:
    ...
    logger.info("⚠️ パフォーマンス・監視モジュールのインポートに失敗しました")
...
class FinalSystemValidator:
    ...
    def check_python_version(self) -> Dict[str, Any]:
        ...
        try:
            ...
            version_ok = python_version.major > required_major or (
                python_version.major == required_major
                and python_version.minor >= required_minor
            )
            ...
            return {
                ...
                "message": (
                    "OK"
                    if version_ok
                    else f"Python {required_major}.{required_minor}以上が必要です"
                ),
            }
    ...
    def check_directory_structure(self) -> Dict[str, Any]:
        """ディレクトリ構造チェック"""
        required_dirs = ["modules", "scripts", "logs", "config", "templates", "static"]

        required_files = [
            "app.py",
            "requirements.txt",
            "config/config.json",
            "modules/enhanced_error_recovery.py",
    ...
    def check_configuration_files(self) -> Dict[str, Any]:
        ...
        # 全体ステータス
        all_configs_ok = config_checks.get("config_json", {}).get(
            "complete", False
        ) and config_checks.get("google_auth", {}).get("ready", False)

        return {"status": all_configs_ok, "details": config_checks}
    ...
    def check_database_setup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": False, "exists": True, "error": str(e)}
    ...
    def check_file_permissions(self) -> Dict[str, Any]:
            ...
            if file_path.exists():
                try:
                    ...
                    writable = (
                        ...
                        if file_path.name in ["config.json", "db.sqlite3"]
                        else True
                    )
        ...
        all_permissions_ok = all(
            check.get("ok", False) for check in permission_checks.values()
        )

        return {"status": all_permissions_ok, "details": permission_checks}
    ...
    def check_external_dependencies(self) -> Dict[str, Any]:
        ...
        return {"status": all_dependencies_ok, "details": dependencies}
    ...
    def calculate_final_score(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "overall_performance_score" in performance:
            score_components["performance"] = min(
                25, performance["overall_performance_score"] / 4
            )
        else:
        ...
        # 本番運用準備判定
        production_readiness = final_score >= 80 and all(
            score >= 15 for score in score_components.values()
        )

        # 推奨事項生成
        recommendations = self.generate_final_recommendations(
            results, score_components, final_score
        )
    ...
    def identify_critical_issues(
        self, results: Dict[str, Any], score_components: Dict[str, float]
    ) -> List[str]:
        """重要課題特定"""
        critical_issues = []
...
def generate_comprehensive_report(results: Dict[str, Any]) -> str:
    ...
    report_lines.append(
        f"  {status_emoji} 前提条件: {'合格' if prerequisites.get('overall_status', False) else '不合格'}"
    )
    ...
    status_emoji = (
        "✅" if int_status == "passed" else "⚠️" if int_status == "warning" else "❌"
    )
    ...
    status_emoji = (
        "✅" if op_status == "healthy" else "⚠️" if op_status == "warning" else "❌"
    )
...
async def main():
    ...
    try:
        ...
        # コンソール出力
        logger.info(readable_report)

        logger.info(f"\n📄 詳細JSON結果: {results_path}")
        logger.info(f"📄 読みやすいレポート: {report_path}")
