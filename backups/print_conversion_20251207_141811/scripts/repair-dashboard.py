...
class RepairDashboard:
    ...
    def init_database(self):
        ...
        try:
            ...
            logger.info(f"{Colors.RED}Error initializing database: {e}{Colors.END}")
    ...
    def load_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"{Colors.RED}Error loading state: {e}{Colors.END}")
            return {}
        ...
        try:
            ...
            if result:
                stats.most_common_failure = (
                    result[0][:50] + "..." if len(result[0]) > 50 else result[0]
                )

            conn.close()
        except Exception as e:
            logger.info(
                f"{Colors.YELLOW}Warning: Could not load extended stats from database: {e}{Colors.END}"
            )
    ...
    def get_system_metrics(self) -> SystemMetrics:
        ...
        try:
            ...
            gh_result = subprocess.run(
                gh_cmd, shell=True, capture_output=True, text=True
            )
    ...
    def store_system_metrics(self, metrics: SystemMetrics):
        ...
        try:
            ...
            logger.info(f"{Colors.YELLOW}Warning: Could not store metrics: {e}{Colors.END}")

    def display_dashboard(self):
        """Display the main dashboard"""
        os.system("clear")

        logger.info(f"{Colors.BLUE}{'='*80}{Colors.END}")
        logger.info(
            f"{Colors.BOLD}{Colors.CYAN}        AUTO-REPAIR SYSTEM DASHBOARD        {Colors.END}"
        )
        logger.info(f"{Colors.BLUE}{'='*80}{Colors.END}")
        logger.info(
            f"{Colors.CYAN}Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{Colors.END}\n"
        )
    ...
    def display_repair_stats(self, stats: RepairStats):
        ...
        logger.info(f"{Colors.YELLOW}📊 REPAIR STATISTICS{Colors.END}")
        logger.info(f"   Total Repairs:     {Colors.BOLD}{stats.total_repairs:,}{Colors.END}")
        logger.info(
            f"   Successful:        {Colors.GREEN}{stats.successful_repairs:,}{Colors.END}"
        )
        logger.info(f"   Failed:            {Colors.RED}{stats.failed_repairs:,}{Colors.END}")
        logger.info(
            f"   Success Rate:      {Colors.GREEN}{stats.success_rate:.1f}%{Colors.END}"
        )
        logger.info(
            f"   Avg Duration:      {Colors.CYAN}{stats.average_duration:.2f}s{Colors.END}"
        )
        logger.info(
            f"   Repairs/Hour:      {Colors.PURPLE}{stats.repairs_per_hour:.2f}{Colors.END}"
        )
        logger.info(
            f"   Last Repair:       {Colors.CYAN}{stats.last_repair_time or 'Never'}{Colors.END}"
        )
        logger.info(
            f"   Common Failure:    {Colors.RED}{stats.most_common_failure}{Colors.END}"
        )
        logger.info()

    def display_system_metrics(self, metrics: SystemMetrics):
        """Display system metrics section"""
        logger.info(f"{Colors.YELLOW}💻 SYSTEM METRICS{Colors.END}")
        logger.info(
            f"   CPU Usage:         {self.colorize_metric(metrics.cpu_usage, 80, 90)}"
        )
        logger.info(
            f"   Memory Usage:      {self.colorize_metric(metrics.memory_usage, 80, 90)}"
        )
        logger.info(
            f"   Disk Usage:        {self.colorize_metric(metrics.disk_usage, 80, 90)}"
        )
        logger.info(f"   System Uptime:     {Colors.CYAN}{metrics.uptime}{Colors.END}")
        logger.info(
            f"   GitHub API:        {self.colorize_api_limit(metrics.github_api_remaining)}"
        )
        logger.info()
    ...
    def display_recent_activity(self):
        ...
        logger.info(f"{Colors.YELLOW}📋 RECENT ACTIVITY (Last 10){Colors.END}")
        ...
        try:
            ...
            if results:
                ...
                logger.info(f"   {Colors.YELLOW}No recent activity recorded{Colors.END}")

            conn.close()
        except Exception as e:
            logger.info(f"   {Colors.RED}Error loading activity: {e}{Colors.END}")

        logger.info()

    def display_failure_patterns(self):
        """Display failure pattern analysis"""
        logger.info(f"{Colors.YELLOW}🔍 FAILURE PATTERNS{Colors.END}")
        ...
        try:
            ...
            if results:
                for row in results:
                    ...
                    logger.info(
                        f"   {Colors.RED}{failure_type}{Colors.END} "
                        f"({count} occurrences, last: {last_occurrence})"
                    )
                    if resolution_strategy:
                        logger.info(
                            f"     └─ Strategy: {Colors.GREEN}{resolution_strategy}{Colors.END}"
                        )
            else:
                logger.info(f"   {Colors.GREEN}No failure patterns detected{Colors.END}")

            conn.close()
        except Exception as e:
            logger.info(f"   {Colors.RED}Error loading patterns: {e}{Colors.END}")
    ...
    def generate_trend_graph(self, output_file: str = "repair_trends.png"):
        ...
        if not HAS_MATPLOTLIB:
            logger.info(f"{Colors.RED}Matplotlib not available for graphing{Colors.END}")
            return
        ...
        try:
            ...
            if HAS_PANDAS:
                ...
                if not df.empty:
                    ...
                    ax1.plot(
                        df["date"], df["total_repairs"], "b-", label="Total Repairs"
                    )
                    ...
                    logger.info(
                        f"{Colors.GREEN}Trend graph saved to: {output_file}{Colors.END}"
                    )
                else:
                    logger.info(f"{Colors.YELLOW}No data available for trending{Colors.END}")

            conn.close()

        except Exception as e:
            logger.info(f"{Colors.RED}Error generating trend graph: {e}{Colors.END}")
    ...
    def export_data(self, format_type: str, output_file: str):
        ...
        try:
            ...
            if format_type.lower() == "json":
                ...
                logger.info(f"{Colors.RED}Unsupported format: {format_type}{Colors.END}")
                return

            conn.close()
            logger.info(f"{Colors.GREEN}Data exported to: {output_file}{Colors.END}")

        except Exception as e:
            logger.info(f"{Colors.RED}Error exporting data: {e}{Colors.END}")
    ...
    def export_json(self, conn: sqlite3.Connection, output_file: str):
        ...
        cursor.execute(
            "SELECT * FROM repair_history ORDER BY timestamp DESC LIMIT 1000"
        )
        ...
        cursor.execute(
            "SELECT * FROM system_metrics ORDER BY timestamp DESC LIMIT 1000"
        )
        columns = [description[0] for description in cursor.description]
        for row in cursor.fetchall():
...
logger = logging.getLogger(__name__)
        ...
        while True:
            ...
            logger.info(f"{Colors.CYAN}Command (q/r/e/g): {Colors.END}", end="", flush=True)

            try:
                command = input().strip().lower()

                if command == "q":
                    logger.info(f"{Colors.YELLOW}Dashboard closed{Colors.END}")
                    break
                elif command == "r":
                    continue
                elif command == "e":
                    format_type = input(
                        f"{Colors.CYAN}Export format (json/csv): {Colors.END}"
                    ).strip()
                    ...
                    if HAS_MATPLOTLIB:
                        ...
                        logger.info(f"{Colors.RED}Matplotlib not available{Colors.END}")
                    input(f"{Colors.CYAN}Press Enter to continue...{Colors.END}")
                else:
                    logger.info(f"{Colors.YELLOW}Unknown command: {command}{Colors.END}")
                    time.sleep(1)
...
def main():
    ...
    parser.add_argument(
        "--graph", "-g", action="store_true", help="Generate trend graph"
    )
    parser.add_argument(
        "--watch",
        "-w",
    ...
    if args.interactive:
        ...
        output_file = (
            args.output
            or f"repair_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{args.export}"
        )
        dashboard.export_data(args.export, output_file)
    elif args.graph:
        output_file = (
            args.output
            or f"repair_trends_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        )
