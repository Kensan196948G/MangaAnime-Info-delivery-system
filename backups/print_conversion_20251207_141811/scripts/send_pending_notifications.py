...
logger = logging.getLogger(__name__)
    ...
    if not sender_password:
        logger.info("❌ Gmail App Passwordが設定されていません")
        return False
...
def main():
    ...
    logger.info("\n📊 現在の状況:")
    logger.info(f"   未通知リリース: {total_pending} 件")
    logger.info(f"   バッチサイズ: {BATCH_SIZE} 件/回")
    logger.info(f"   最大送信数: {MAX_NOTIFICATIONS} 件")
    logger.info()

    if total_pending == 0:
        logger.info("✅ 送信待ちの通知はありません")
        return
    ...
    while sent_count < MAX_NOTIFICATIONS and sent_count < total_pending:
        ...
        logger.info(f"   {len(releases)} 件を送信...")
        ...
        if send_notification_email(releases, config):
            ...
            logger.info(f"   ✅ 送信成功 (累計: {sent_count} 件)")
        else:
            logger.info("   ❌ 送信失敗")
            break
        ...
        if sent_count < MAX_NOTIFICATIONS and sent_count < total_pending:
            logger.info(f"   ⏸️  {DELAY_BETWEEN_BATCHES}秒待機中...")
            time.sleep(DELAY_BETWEEN_BATCHES)

    # 結果サマリー
    logger.info("\n" + "=" * 60)
    logger.info("📊 送信結果:")
    logger.info(f"   送信済み: {sent_count} 件")
    logger.info(f"   残り: {total_pending - sent_count} 件")
