...
class ConfigHelper:
    ...
    def email_subject_prefix(self) -> str:
        """メール件名のプレフィックス"""
        return self._config.get('notifications', {}).get('email', {}).get(
    ...
    def calendar_id(self) -> str:
        """GoogleカレンダーID"""
        return self._config.get('notifications', {}).get('calendar', {}).get('calendar_id', 'primary')

    @property
    def calendar_title_format(self) -> str:
        """カレンダーイベントのタイトルフォーマット"""
        return self._config.get('notifications', {}).get('calendar', {}).get(
    ...
    def api_anilist_rate_limit(self) -> int:
        ...
        return self._config.get('api', {}).get('rate_limiting', {}).get(
            'anilist_requests_per_minute', 85
        )
...
if __name__ == '__main__':
    ...
    print(f"NG Keywords: {', '.join(config.ng_keywords)}")
    print(f"Anime Sources: {', '.join(config.anime_sources)}")
    print(f"Manga Sources: {', '.join(config.manga_sources)}")
