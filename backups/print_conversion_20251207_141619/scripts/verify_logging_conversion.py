...
class LoggingConversionVerifier:
    ...
    def scan_project(self) -> Dict:
        """プロジェクト全体をスキャン"""
        print("=" * 80)
        print("Logging Conversion Verification")
        print("=" * 80)
        ...
        for file_path in python_files:
            ...
            if result.get('issues'):
                ...
                print(f"⚠ Issues found: {result['file']}")
                for issue in result['issues']:
                    print(f"  - {issue['type']}")
                    if 'lines' in issue:
                        print(f"    Lines: {', '.join(map(str, issue['lines'][:5]))}" +
    ...
    def print_report(self):
        ...
        print()
        print(f"Remaining Print Statements: {summary['total_print_statements']}")
        print(f"Files with Prints:          {summary['files_with_prints']}")
        print(f"Files without Logger:       {summary['files_without_logger']}")
        print("=" * 80)
    ...
    def generate_fix_script(self, output_file: str = None):
        ...
        if self.results['total_print_statements'] == 0:
            print("No print statements to fix")
            return
...
print(f"Found {len(files_to_fix)} files with print statements to fix")
"""
