...
def add_login_decorators(file_path):
    ...
    if 'from flask_login import' not in content:
        ...
        print("  - flask_login インポート追加")
    else:
        print("  - flask_login インポート: すでに存在")
        ...
        if re.search(pattern, content):
            ...
            if route_match:
                ...
                if '@login_required' not in route_section:
                    ...
                    print(f"  - {description}: すでに保護されています")
    ...
    if content != original_content:
        ...
        print(f"💾 更新を保存: {file_path}")
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(content)

        print("\n✅ 完了!")
        print("\n⚠️  次のステップ:")
        print("1. python3 app/web_app.py でインポートエラーがないか確認")
        print("2. pytest tests/ でテスト実行")
        print("3. git diff app/web_app.py で変更内容を確認")
        print("4. 問題があれば app/web_app.py.backup から復元可能")
    else:
...
if __name__ == '__main__':
    ...
    if not web_app_path.exists():
        print(f"❌ エラー: {web_app_path} が見つかりません")
        sys.exit(1)
