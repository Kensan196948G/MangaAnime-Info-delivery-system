...
def create_database_tables():
    ...
    try:
        ...
        ''')
        print("  ✓ users.locked_until カラム追加")
    except sqlite3.OperationalError:
        print("  - users.locked_until カラムは既に存在します")

    try:
        cursor.execute('''
            ALTER TABLE users ADD COLUMN failed_attempts INTEGER DEFAULT 0
        ''')
        print("  ✓ users.failed_attempts カラム追加")
    except sqlite3.OperationalError:
        print("  - users.failed_attempts カラムは既に存在します")

    try:
        cursor.execute('''
            ALTER TABLE users ADD COLUMN last_login DATETIME
        ''')
        print("  ✓ users.last_login カラム追加")
    except sqlite3.OperationalError:
        print("  - users.last_login カラムは既に存在します")
...
def insert_sample_data():
    ...
    print(f"  ✓ {len(sample_logs)} 件の監査ログを追加")
    ...
    print(f"  ✓ {len(sample_keys)} 件のAPIキーを追加")
...
def verify_installation():
    ...
    if 'audit_logs' in tables:
        ...
        print(f"  ✓ audit_logs テーブル存在 ({count} 件)")
    else:
        print("  ✗ audit_logs テーブル不足")

    if 'api_keys' in tables:
        cursor.execute('SELECT COUNT(*) FROM api_keys')
        count = cursor.fetchone()[0]
        print(f"  ✓ api_keys テーブル存在 ({count} 件)")
    else:
        print("  ✗ api_keys テーブル不足")
    ...
    for col in required_columns:
        if col in columns:
            print(f"  ✓ users.{col} カラム存在")
        else:
            print(f"  ✗ users.{col} カラム不足")
...
def print_next_steps():
    """次のステップを表示"""
    print("=" * 60)
    print("管理者ダッシュボードセットアップ完了!")
    print("=" * 60)
    print()
    print("次のステップ:")
    print()
    print("1. ブループリントを登録:")
    print("   app/__init__.py または app/web_app.py に以下を追加")
    print()
    print("   from app.routes.admin_dashboard import admin_dash_bp")
    print("   app.register_blueprint(admin_dash_bp)")
    print()
    print("2. ナビゲーションメニューを追加:")
    print("   templates/base.html の管理者メニューに")
    print("   ダッシュボードリンクを追加")
    print()
    print("3. アプリケーションを起動:")
    print("   python app/web_app.py")
    print()
    print("4. ダッシュボードにアクセス:")
    print("   http://localhost:5000/admin/dashboard")
    print()
    print("詳細は docs/ADMIN_DASHBOARD_IMPLEMENTATION.md を参照")
    print("=" * 60)
