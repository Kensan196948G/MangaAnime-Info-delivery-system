...
def scan_file(file_path: Path) -> List[Dict]:
    ...
    try:
        ...
        print(f"⚠️  エラー: {file_path}: {e}", file=sys.stderr)
...
def scan_project(project_root: Path) -> Dict[str, List[Dict]]:
    ...
    print(f"🔍 スキャン中... ({len(python_files)}ファイル)")
...
def generate_report(results: Dict[str, List[Dict]]) -> None:
    ...
    print(f"📁 影響を受けるファイル: {len(results)}件\n")

    # ファイルごとに表示
    for file_path, vulnerabilities in sorted(results.items()):
        print("-" * 80)
        print(f"📄 {file_path}")
        print(f"   脆弱性: {len(vulnerabilities)}件\n")

        for vuln in vulnerabilities:
            print(f"   🔴 行 {vuln['line']}: {vuln['description']}")
            print(f"      種類: {vuln['type']}")
            print(f"      コード: {vuln['code'][:80]}")
            print()
