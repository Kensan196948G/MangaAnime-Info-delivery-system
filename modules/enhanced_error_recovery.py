...
class EnhancedErrorRecovery:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.max_consecutive_errors = config.get('error_recovery', {}).get('max_consecutive_errors', 5)
    ...
    def _initialize_component_health(self):
        ...
        components = [
            'anilist_api', 'manga_rss', 'gmail_api', 
            'calendar_api', 'database', 'web_ui', 
            'config_manager', 'notification_system'
        ]
    ...
    def record_error(self, component: str, error_type: str, message: str, 
                    ...
                    exception: Optional[Exception] = None) -> ErrorEvent:
        """
    ...
    def _update_component_health(self, component: str, error_event: ErrorEvent):
        ...
        with self.health_lock:
            ...
            if (health.consecutive_errors >= self.max_consecutive_errors or 
                health.health_score < self.health_threshold):
        ...
        try:
            ...
            if handler:
                ...
                if success:
                    ...
                    self.logger.info(f"Recovery successful for {component}:{error_type} using {strategy.value}")
                else:
                    self.failed_recoveries += 1
                    self.logger.warning(f"Recovery failed for {component}:{error_type} using {strategy.value}")
    ...
    def _retry_recovery(self, error_event: ErrorEvent) -> bool:
        ...
        self.logger.info(f"Applying retry recovery for {error_event.component}:{error_event.error_type}")
    ...
    def _fallback_recovery(self, error_event: ErrorEvent) -> bool:
        ...
        self.logger.info(f"Applying fallback recovery for {error_event.component}:{error_event.error_type}")
    ...
    def _reset_recovery(self, error_event: ErrorEvent) -> bool:
        ...
        self.logger.info(f"Applying reset recovery for {error_event.component}:{error_event.error_type}")
    ...
    def _manual_recovery(self, error_event: ErrorEvent) -> bool:
        ...
        self.logger.critical(f"Manual intervention required for {error_event.component}:{error_event.error_type}")
    ...
    def _perform_health_check(self):
        ...
        # Log health summary
        self.logger.debug(f"Health check complete. Total components: {len(self.component_health)}, "
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        return {
            'overall_health_score': total_health_score / len(self.component_health) if self.component_health else 1.0,
            ...
            'error_recovery_stats': {
                ...
                'recovery_success_rate': (
                    self.successful_recoveries / max(self.successful_recoveries + self.failed_recoveries, 1)
                ),
...
def record_error(component: str, error_type: str, message: str, 
                severity: ErrorSeverity = ErrorSeverity.MEDIUM,
                details: Optional[Dict[str, Any]] = None,
                exception: Optional[Exception] = None) -> Optional[ErrorEvent]:
