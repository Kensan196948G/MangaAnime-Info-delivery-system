...
class AniListClient:
    ...
    async def _enforce_rate_limit(self):
        ...
        async with self.rate_limit_lock:
            ...
            recent_requests = [
                ts for ts in self.request_timestamps if now - ts < 10
            ]  # Last 10 seconds
            ...
            if current_requests >= effective_limit * self.BURST_THRESHOLD:
                ...
                if adaptive_delay > 0:
                    self.logger.debug(
                        f"Adaptive throttling: sleeping {adaptive_delay:.1f}s (current limit: {effective_limit})"
                    )
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                timeout = aiohttp.ClientTimeout(
                    total=self.timeout, connect=10, sock_read=self.timeout - 10
                )
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "burst_mode_active": len(self.request_timestamps)
            >= self.RATE_LIMIT * self.BURST_THRESHOLD,
            "performance_grade": self._calculate_performance_grade(
                avg_response_time, self.error_count
            ),
    ...
    async def get_studio_anime(self, studio_name: str, limit: int = 20) -> List[AniListWork]:
        ...
        try:
            data = await self._make_request(
                graphql_query, {"search": studio_name, "perPage": min(limit, 50)}
            )
    ...
    async def get_genre_anime(
        self, genre: str, limit: int = 20, year: int = None
    ) -> List[AniListWork]:
        """
        Get anime by genre.
        ...
        try:
            data = await self._make_request(
                graphql_query,
                {"genre": genre, "perPage": min(limit, 50), "seasonYear": year},
            )
    ...
    async def get_streaming_anime(self, platform: str, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            data = await self._make_request(
                graphql_query, {"perPage": min(limit * 3, 100)}
            )  # Get more to filter
            media_list = data.get("Page", {}).get("media", [])
...
class AniListCollector:
    ...
    def _should_filter_work(self, anilist_work: AniListWork) -> bool:
        ...
        for keyword in self.ng_keywords:
            for title in [
                anilist_work.title_romaji,
                anilist_work.title_english,
                anilist_work.title_native,
            ]:
        ...
        if anilist_work.description:
            for keyword in self.ng_keywords:
                if keyword.lower() in anilist_work.description.lower():
                    self.logger.debug(
                        f"Filtered by description keyword '{keyword}': {anilist_work.title_romaji}"
                    )
                    return True
