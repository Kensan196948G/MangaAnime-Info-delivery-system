...
class ErrorAnalyzer:
    ...
    def _get_recent_errors(self, error_key: str, hours: int = 1) -> List[ErrorContext]:
        ...
        return [
            error for error in self.error_patterns[error_key]
            if error.timestamp > cutoff_time
        ]
    
    def record_recovery_success(self, error_context: ErrorContext, 
                               recovery_action: RecoveryAction, success: bool):
        """復旧の成功/失敗を記録"""
        ...
        self.logger.debug(f"Recovery success rate updated: {key}:{strategy} = {new_rate:.2f}")
...
class RecoveryExecutor:
    ...
    async def execute_recovery(self, error_context: ErrorContext, 
    ...
    async def _immediate_retry(self, target_function: Callable, 
                             recovery_action: RecoveryAction,
    ...
    async def _exponential_backoff_retry(self, target_function: Callable,
                                       recovery_action: RecoveryAction,
                                       error_context: ErrorContext) -> bool:
        """指数バックオフリトライ"""
        ...
        for attempt in range(recovery_action.max_attempts):
            try:
                self.logger.info(f"Exponential backoff retry attempt {attempt + 1}, delay: {delay:.1f}s")
                ...
                self.logger.warning(f"Exponential backoff attempt {attempt + 1} failed: {e}")
                delay *= 2
                continue
        
        return False
    
    async def _circuit_breaker_recovery(self, target_function: Callable,
                                      recovery_action: RecoveryAction, 
                                      error_context: ErrorContext) -> bool:
        """サーキットブレーカー方式の復旧"""
        self.logger.info(f"Circuit breaker activated for {recovery_action.delay_seconds}s")
        ...
        try:
            ...
            if result:
                ...
                self.logger.warning("Circuit breaker test failed - keeping circuit open")
                return False
...
class AdaptiveScheduler:
    ...
    def calculate_optimal_interval(self, task_name: str, 
        ...
        if len(self.task_performance_history[task_name]) > 50:
            self.task_performance_history[task_name] = self.task_performance_history[task_name][-50:]
        ...
        avg_execution_time = sum(e['execution_time'] for e in recent_entries) / len(recent_entries)
        avg_error_rate = sum(e['error_rate'] for e in recent_entries) / len(recent_entries)
        avg_success_rate = sum(e['success_rate'] for e in recent_entries) / len(recent_entries)
        avg_load_factor = sum(e['load_factor'] for e in recent_entries) / len(recent_entries)
        ...
        self.logger.debug(f"Optimal interval for {task_name}: {optimal_interval:.1f}s "
    ...
    def get_scheduler_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'performance_history_counts': {
                task: len(history) for task, history in self.task_performance_history.items()
            }
        }
...
class SmartErrorRecoverySystem:
    ...
    async def handle_error(self, error: Exception, service_name: str, 
                         operation: str, target_function: Callable,
        ...
        self.logger.warning(f"Error handling started: {service_name}:{operation} - {error}")
        ...
        if recovery_action.strategy != RecoveryStrategy.MANUAL_INTERVENTION:
            ...
            self.logger.critical(f"Manual intervention required: {service_name}:{operation}")
        ...
        # データベース更新
        self._update_error_log(error_context, recovery_success, recovery_action.strategy.value)
        
        self.logger.info(f"Error handling completed: {service_name}:{operation} - "
    ...
    def _log_error_to_db(self, error_context: ErrorContext, 
        ...
        try:
            ...
            """, (
                ...
                json.dumps(error_context.context_data) if error_context.context_data else None,
                recovery_success,
    ...
    def _update_error_log(self, error_context: ErrorContext, 
                         recovery_success: bool, strategy: str):
        """エラーログの更新"""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
    ...
    def optimize_schedule(self, task_name: str, default_interval: float,
                         execution_time: float, error_rate: float,
                         success_rate: float, load_factor: float = 0.5) -> float:
        """
        スケジュール最適化
...
def initialize_error_recovery(db_path: Optional[str] = None) -> SmartErrorRecoverySystem:
    """エラー復旧システムの初期化"""
...
async def handle_system_error(error: Exception, service_name: str, operation: str,
...
def optimize_task_schedule(task_name: str, default_interval: float,
                          execution_time: float, error_rate: float,
                          success_rate: float, load_factor: float = 0.5) -> float:
    """
    タスクスケジュール最適化関数
    ...
    if recovery_system:
        return recovery_system.optimize_schedule(
            task_name, default_interval, execution_time, 
            error_rate, success_rate, load_factor
        )
    else:
