...
class FilteringConfig:
    ...
    ng_keywords: List[str] = field(default_factory=lambda: [
        "エロ", "R18", "成人向け", "BL", "百合", "ボーイズラブ", 
        "アダルト", "18禁", "官能", "ハーレクイン"
    ])
    ng_genres: List[str] = field(default_factory=lambda: [
        "Hentai", "Ecchi"
    ])
...
class CalendarNotificationConfig:
    ...
    color_by_type: Dict[str, str] = field(default_factory=lambda: {
        "anime": "blue",
        "manga": "green"
    })
...
class ConfigManager:
    ...
    def get_log_format(self) -> str:
        """Get log format."""
        return self._config_data.get('logging', {}).get('format', '%(asctime)s - %(name)s - %(levelname)s - %(message)s')
