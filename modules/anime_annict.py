...
async def main():
    ...
    if not annict_config.get('access_token') and not annict_config.get('api_key'):
        print("⚠️  Please set your Annict access token in config.json")
        print("   Get it from: https://annict.com/settings/apps")
        return

    print("🔍 Testing Annict API integration...")
    ...
    if result['works']:
        print(f"\n📺 Sample work:")
        print(json.dumps(result['works'][0], indent=2, ensure_ascii=False))
