...
if __name__ == "__main__":
    ...
    # 各種設定の取得
    print("\n--- Configuration ---")
    print(f"AniList enabled: {config.is_enabled('anime_sources.anilist')}")
    print(f"AniList URL: {config.get_api_url('anime_sources.anilist')}")
    print(f"NG Keywords: {config.get_ng_keywords()}")
    print(f"Notification emails: {config.get_notification_emails()}")
    ...
    # ネストされた設定の取得
    print("\n--- Nested Config ---")
    print(f"Schedule timezone: {config.get('schedule.timezone')}")
    print(f"Calendar ID: {config.get('notification.calendar.calendar_id')}")
