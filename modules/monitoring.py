    ...
    def get_metrics_summary(self, hours: int = 1) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            if recent_app:
                ...
                summary["application_metrics"]["total_api_calls"] = (
                    max(api_calls) if api_calls else 0
                )
...
class SystemMonitor:
    ...
    def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_metrics_export": (
                datetime.fromtimestamp(self._last_metrics_export).isoformat()
                if self._last_metrics_export
                else None
            ),
    ...
    def _get_critical_issues(self) -> List[str]:
        ...
        for service, metrics in self.collection_performance_tracker.items():
            if (
                metrics["requests"] > 0
                or metrics.get("feeds_processed", 0) > 0
                or metrics.get("queries", 0) > 0
            ):
                error_rate = metrics["errors"] / max(
                    metrics["requests"]
                    or metrics.get("feeds_processed", 0)
                    or metrics.get("queries", 0),
                    1,
                )
    ...
    def _get_performance_trend(self) -> str:
        ...
        recent_avg = sum(p.get("overall_score", 0.5) for p in recent_performance) / len(
            recent_performance
        )
        older_avg = sum(p.get("overall_score", 0.5) for p in older_performance) / len(
            older_performance
        )
    ...
    def record_collection_performance(
        self, service: str, operation_type: str, response_time: float, success: bool
    ):
        """
        ...
        if total_operations > 0:
            current_avg = (
                tracker.get("avg_response_time", 0.0)
