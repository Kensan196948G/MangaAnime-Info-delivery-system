...
from .models import Work, Release, AniListWork, RSSFeedItem, WorkType, ReleaseType, DataSource
...
class BackendValidator:
    ...
    def __init__(self, config_manager=None, timeout: int = 30):
        ...
        self.test_config = {
            ...
            'rss': {'test_feeds': ['https://httpbin.org/xml'], 'enable_parser_test': True},
    ...
    async def run_comprehensive_validation(self) -> ValidationReport:
        ...
        self.logger.info(f"Validation completed: {passed_tests}/{len(test_results)} tests passed "
    ...
    def _gather_system_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'platform': 'Unknown', 'note': 'psutil not available for detailed system info'}
    ...
    async def _test_db_integrity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            unique_constraint_enforced = release_id_1 == release_id_2  # Should return same ID due to UNIQUE constraint
    ...
    async def _test_anilist_module(self) -> List[TestResult]:
        ...
        results.append(await self._run_test("AniList API Connectivity", self._test_anilist_connectivity))
        
        # Test 3: Rate limiting
        if self.test_config['anilist']['enable_rate_limit_test']:
            results.append(await self._run_test("AniList Rate Limiting", self._test_anilist_rate_limiting))
        
        # Test 4: Circuit breaker
        results.append(await self._run_test("AniList Circuit Breaker", self._test_anilist_circuit_breaker))
    ...
    async def _test_anilist_init(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'initial_performance_stats': perf_stats
        }
    ...
    async def _test_anilist_circuit_breaker(self) -> Dict[str, Any]:
        ...
        client = AniListClient(timeout=1, retry_attempts=1)  # Very short timeout to trigger failures
    ...
    async def _test_filter_module(self) -> List[TestResult]:
        ...
        if self.test_config['filter']['enable_performance_test']:
            results.append(await self._run_test("Filter Performance", self._test_filter_performance))
    ...
    async def _test_filter_performance(self) -> Dict[str, Any]:
        ...
        class TestFilterConfig:
            def get_ng_keywords(self):
                return ['blocked', 'adult', 'nsfw', 'test1', 'test2'] * 10  # 50 keywords
    ...
    async def _test_fuzzy_matching(self) -> Dict[str, Any]:
        ...
        fuzzy_filter = ContentFilter(TestFilterConfig(), enable_fuzzy_matching=True, similarity_threshold=0.8)
        ...
        return {
            ...
            'fuzzy_caught_more': sum(1 for f in fuzzy_results if f['filtered']) > sum(1 for e in exact_results if e['filtered'])
        }
    ...
    async def _test_integration(self) -> List[TestResult]:
        ...
        results.append(await self._run_test("DB-Filter Integration", self._test_db_filter_integration))
    ...
    async def _test_e2e_workflow(self) -> Dict[str, Any]:
        ...
        test_releases_in_unnotified = [
            ...
            if r['work_id'] in created_work_ids
        ]
        ...
        return {
            ...
            'items_filtered': len([item for item in processed_items if item['status'] == 'filtered']),
