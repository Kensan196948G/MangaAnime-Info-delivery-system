...
class EmailSender:
    def __init__(self, config_path: str = 'config.json'):
        ...
        try:
            ...
            logger.info(f"Email sender initialized in {'test' if self.test_mode else 'production'} mode")
    ...
    def send_release_notification(self, releases: list) -> dict:
        ...
        try:
            ...
            for release in releases[:10]:  # Limit to first 10
                ...
                body_lines.append(f"• {title} - 第{episode}話/巻 ({platform}) - {release_date}")
            ...
            if self.test_mode:
                logger.info(f"TEST MODE: Would send release notification with {len(releases)} releases")
                return {
                    'success': True,
