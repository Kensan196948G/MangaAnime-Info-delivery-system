...
class SecurityCompliance:
    ...
    def _check_nested_dict_for_sensitive_data(self, data: Dict, sensitive_keys: List[str], 
                                            findings: List[SecurityFinding], file_path: str) -> None:
        """Recursively check dictionary for sensitive data"""
        for key, value in data.items():
            if isinstance(value, dict):
                self._check_nested_dict_for_sensitive_data(value, sensitive_keys, findings, file_path)
    ...
    def _generate_security_recommendations(self) -> List[str]:
        ...
        if len(self.findings) == 0:
            ...
            recommendations.append("High number of findings. Consider security training for development team.")
    ...
    def _calculate_security_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'compliance_score': sum(r.score for r in self.compliance_results) / len(self.compliance_results) if self.compliance_results else 0,
            'security_coverage': len([r for r in self.compliance_results if r.passed]) / len(self.compliance_results) if self.compliance_results else 0,
