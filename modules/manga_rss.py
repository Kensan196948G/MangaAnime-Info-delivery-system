...
class MangaRSSCollector:
    ...
    def collect(self) -> List[Dict[str, Any]]:
        ...
        manga_feeds = [feed for feed in self.enabled_feeds 
        ...
        for attempt in range(retry_count):
            try:
                ...
                if response_time > 5.0:
                    self.logger.warning(f"{feed_name}RSSの応答が遅延しています ({response_time:.1f}s)")
                ...
                self.logger.info(f"{feed_name}から{len(items)}件のアイテムを収集 ({response_time:.1f}s)")
                return items
                
            except requests.exceptions.Timeout:
                self.logger.error(f"{feed_name} RSS接続がタイムアウトしました (試行 {attempt + 1}/{retry_count})")
                if attempt < retry_count - 1:
                ...
                self.logger.error(f"{feed_name} RSS接続に失敗しました (試行 {attempt + 1}/{retry_count})")
                if attempt < retry_count - 1:
                    self.logger.info(f"{retry_delay}秒後に再試行します...")
                    time.sleep(retry_delay)
                    continue
                ...
                self.logger.error(f"{feed_name}で予期しないエラー: {e} (試行 {attempt + 1}/{retry_count})")
                if attempt < retry_count - 1:
                    time.sleep(retry_delay)
                    continue
        ...
        for entry in feed_data.entries:
            try:
                ...
                if rss_item:
                    ...
                    if work_info:
                        # 統一形式に変換
                        normalized_item = self._normalize_manga_item_enhanced(work_info, rss_item, feed_name)
                        if normalized_item:
    ...
    def _is_feed_healthy(self, response, feed_name: str) -> bool:
        ...
        try:
            ...
            if len(response.content) < 100:
                self.logger.warning(f"レスポンスサイズが小さすぎます ({feed_name}): {len(response.content)} bytes")
                return False
    ...
    def _extract_work_info_enhanced(self, rss_item: RSSFeedItem, source_name: str) -> Optional[Dict[str, Any]]:
        """
        強化版作品情報抽出
    ...
    def _normalize_manga_item_enhanced(self, work_info: Dict[str, Any], 
                                     rss_item: RSSFeedItem, source_name: str) -> Optional[Dict[str, Any]]:
        """
        強化版マンガ情報正規化
        ...
        try:
            ...
            release_info = {
                'type': work_info.get('release_type', ReleaseType.VOLUME).value if work_info.get('release_type') else 'volume',
    ...
    def _normalize_manga_item(self, work_info: Dict[str, Any], 
                             rss_item: RSSFeedItem, source_name: str) -> Optional[Dict[str, Any]]:
        """
...
class DuplicateDetector:
    ...
    def _normalize_title(self, title: str) -> str:
        ...
        normalized = re.sub(r'[！-～]', lambda m: chr(ord(m.group(0)) - 0xFEE0), normalized)  # 全角英数→半角
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_title_variants': sum(len(variants) for variants in self.title_variations.values())
        }
...
class BookWalkerRSSCollector(MangaRSSCollector):
    ...
    def collect(self) -> List[Dict[str, Any]]:
        ...
        bookwalker_feeds = [
            feed for feed in self.enabled_feeds 
            if 'bookwalker' in feed.get('name', '').lower() or 
               'bookwalker' in feed.get('url', '').lower()
        ]
...
class DAnimeRSSCollector(MangaRSSCollector):
    ...
    def collect(self) -> List[Dict[str, Any]]:
        ...
        # dアニメストア用のフィードのみを処理
        danime_feeds = [feed for feed in self.enabled_feeds 
    ...
    def _collect_danime_feed_enhanced(self, feed_url: str, feed_name: str) -> List[Dict[str, Any]]:
        ...
        for item in items:
            ...
            # エピソード情報を解析
            episode_info = self._extract_episode_info(item.get('title', ''), item.get('description', ''))
            if episode_info:
                item['metadata']['episode_info'] = episode_info
            
            # 配信日時情報を正規化
            streaming_date = self._normalize_streaming_date(item.get('releases', [{}])[0].get('date'))
            if streaming_date:
    ...
    def get_health_report(self) -> Dict[str, Any]:
        ...
        avg_response_time = 0
        if self.feed_health:
            response_times = [health.average_response_time for health in self.feed_health.values() if health.average_response_time > 0]
            if response_times:
...
class FeedHealth:
    ...
    def average_response_time(self) -> float:
        ...
        return self.total_response_time / (self.total_requests - self.total_failures) if (self.total_requests - self.total_failures) > 0 else 0.0
...
class EnhancedRSSParser:
    ...
    def extract_title(self, raw_title: str) -> str:
        ...
        prefixes = ['新刊：', 'New:', 'NEW:', '更新：', 'Update:', 'Latest:', 'New Release:']
    ...
    def extract_number_and_type(self, text: str) -> Tuple[Optional[str], Optional[ReleaseType]]:
