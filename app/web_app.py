...
# CORS headers configuration for API endpoints
@app.after_request
...
def create_api_response(success, message, data=None, error=None, status_code=200):
    ...
    Args:
        ...
        error (dict, optional): Error information with 'message', 'details', 'code'
...
def api_refresh_data():
    """API endpoint to refresh data by running insert_sample_data.py
    ...
    try:
        logger.info("データ更新リクエストを受信しました")

        # Get the absolute path to insert_sample_data.py
        ...
        logger.info(f"データ投入スクリプトを実行中: {script_path}")
        ...
        try:
            ...
            logger.warning(f"統計情報の取得に失敗: {e}")

        # Save update time and status
        update_data = {
            "last_update": update_time,
            "success": result.returncode == 0,
            "output": result.stdout[:1000] if result.stdout else "",  # Limit output size
            "error": result.stderr[:1000] if result.returncode != 0 and result.stderr else None,
            "stats": stats
        }
        ...
        if result.returncode == 0:
            ...
            logger.error(f"データ更新エラー: {result.stderr}")
            return create_api_response(
                success=False,
                ...
                error={
                    "message": "データ更新中にエラーが発生しました",
                    "details": result.stderr[:500] if result.stderr else "不明なエラー",
                    "code": "UPDATE_FAILED"
                },
                status_code=500
            )
...
def api_data_status():
    """API endpoint for data status information"""
    try:
        ...
        if os.path.exists(update_file):
            try:
                with open(update_file, "r", encoding="utf-8") as f:
                    ...
                    update_success = update_data.get("success")
            except Exception as e:
                logger.warning(f"更新情報の読み込みエラー: {e}")
        ...
        stats = {
            ...
            "recent_releases": conn.execute(
                "SELECT COUNT(*) FROM releases WHERE date(created_at) >= date('now', '-7 days')"
            ).fetchone()[0],
        }
        ...
        platform_stats = [{"platform": p[0], "count": p[1]} for p in platforms]
        ...
        logger.error(f"データステータス取得エラー: {e}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def api_test_notification():
    ...
    try:
        ...
        # デバッグ情報
        logger.info(f"Environment variables loaded: address={bool(gmail_address)}, password={bool(gmail_password)}")

        # 設定ファイルからメールアドレス取得（フォールバック）
        config = load_config()
        ...
        if not from_email or not to_email:
            error_details = {
                "env_gmail_address": bool(os.getenv('GMAIL_ADDRESS')),
                "env_gmail_sender": bool(os.getenv('GMAIL_SENDER_EMAIL')),
                "config_from_email": bool(gmail_config.get('from_email')),
                "config_notification_email": bool(config.get('notification_email')),
            }
            logger.error(f"Email address not configured: {error_details}")
            return jsonify({
                "success": False,
        ...
        logger.info(f"Preparing to send test email from {from_email} to {to_email}")
        ...
        return jsonify({
            ...
            "details": str(e),
            "help": "python-dotenv モジュールがインストールされているか確認してください"
        }), 500
    except Exception as e:
...
def auto_populate_data():
    """Automatically populate sample data on startup if database is empty"""
    import subprocess
    import sys
    ...
    try:
        ...
        if work_count == 0:
            logger.info("データベースが空です。サンプルデータを自動投入します...")
            ...
            if os.path.exists(script_path):
                logger.info(f"サンプルデータ投入スクリプトを実行: {script_path}")
                result = subprocess.run(
                    [sys.executable, script_path],
                    capture_output=True,
                    text=True,
                ...
                if result.returncode == 0:
                    ...
                    logger.error(f"サンプルデータ投入エラー: {result.stderr}")
            else:
                logger.warning(f"サンプルデータ投入スクリプトが見つかりません: {script_path}")
    except Exception as e:
        logger.error(f"自動データ投入エラー: {e}")
...
if __name__ == "__main__":
    ...
    logger.info("Flask Webアプリケーションを起動します...")
    logger.info("URL: http://localhost:3030")
