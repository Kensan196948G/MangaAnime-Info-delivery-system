...
def readiness_check():
    ...
    is_ready = db_status["status"] == "healthy" and config_status["status"] in [
        "healthy",
        "degraded",
    ]
    ...
    response = {
        ...
        "timestamp": datetime.now().isoformat(),
        "checks": {"database": db_status, "config": config_status},
        "uptime": uptime,
    }
...
def detailed_health_check():
    ...
    try:
        ...
        disk_status = {"status": "unknown", "error": str(e)}
    ...
    overall_healthy = (
        ...
        and config_status["status"] in ["healthy", "degraded"]
        and disk_status["status"] != "unhealthy"
    )
...
def prometheus_metrics():
    ...
    # Database health
    metrics.append(
        "# HELP mangaanime_database_healthy Database health status (1=healthy, 0=unhealthy)"
    )
