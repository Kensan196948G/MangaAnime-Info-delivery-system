...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if not current_user.is_admin:
            logger.warning(
                f"管理者権限のないユーザー '{current_user.username}' がアクセスを試みました"
            )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if is_locked:
                ...
                # 監査ログ記録
                try:
                    from modules.audit_log import AuditEventType, audit_logger
...
def reset_password(token):
    ...
    if request.method == "POST":
        ...
        if not password or len(password) < 8:
            flash(
                "パスワードは8文字以上で入力してください。セキュリティ強化のため、大文字・小文字・数字を含めることを推奨します。",
                "warning",
            )
