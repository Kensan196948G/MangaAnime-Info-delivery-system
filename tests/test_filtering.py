...
class TestContentFiltering:
    ...
    def test_ng_genre_filtering(self, test_config):
        ...
        for genres, should_be_filtered in test_cases:
            ...
            assert is_filtered == should_be_filtered, f"Genres {genres} filtering result mismatch"
    ...
    def test_case_insensitive_filtering(self, test_config):
        ...
        for text, should_be_filtered in test_cases:
            ...
            is_filtered = any(
                ...
                keyword in text  # For Japanese characters
...
class TestFilteringPerformance:
    ...
    def test_bulk_filtering_performance(self, test_config, test_data_generator):
        ...
        # Verify performance
        assert filtering_time < 2.0, f"Filtering 1000 items took {filtering_time:.2f}s, should be under 2.0s"
    ...
    def _should_filter_media_fast(self, media, ng_keywords, ng_genres):
        ...
        description = media.get("description", "")
        if description and any(keyword.lower() in description.lower() for keyword in ng_keywords):
            return True
    ...
    def test_regex_vs_string_matching_performance(self, test_config):
        ...
        pattern = re.compile('|'.join(re.escape(keyword.lower()) for keyword in ng_keywords), re.IGNORECASE)
        ...
        assert string_results == regex_results, "String matching and regex should produce same results"
...
class TestFilteringEdgeCases:
    ...
    def test_empty_input_filtering(self, test_config):
        ...
        test_cases = [
            {"title": {"romaji": None}, "description": None, "genres": None, "tags": None},
    ...
    def test_unicode_and_special_characters(self, test_config):
        ...
        for text, should_be_filtered in test_cases:
            ...
            assert is_filtered == should_be_filtered, f"Unicode filtering for '{text}' failed"
    ...
    def test_very_long_text_filtering(self, test_config):
        ...
        assert is_filtered
        assert (end_time - start_time) < 0.1, "Long text filtering with NG keyword should be fast"
    ...
    def test_japanese_text_normalization(self, test_config):
        ...
        for text, should_be_filtered in test_cases:
            ...
            if not is_filtered and any(char in 'あいうえおかきくけこさしすせそたちつてとなにぬねのはひふへほまみむめもやゆよらりるれろわをん' for char in text):
            ...
            if text in ["えろ"]:  # Special case for hiragana that should match
                assert is_filtered, f"Japanese text filtering for '{text}' should handle hiragana/katakana conversion"
            else:
                assert is_filtered == should_be_filtered, f"Japanese text filtering for '{text}' failed"
    ...
    def _safe_filter_media(self, media, ng_keywords, ng_genres):
        ...
        try:
            ...
            if description and self._contains_ng_keywords_safe(description, ng_keywords):
                return True
            
            # Safely get genres
            genres = media.get("genres") or []
            if isinstance(genres, list) and any(genre in ng_genres for genre in genres if genre):
                return True
