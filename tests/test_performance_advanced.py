...
class TestAdvancedDatabasePerformance:
    ...
    @pytest.mark.slow
    def test_database_scalability_analysis(self, temp_db, performance_monitor, benchmark_config):
        ...
        for size in dataset_sizes:
            with performance_monitor.monitor_test(f"db_scalability_{size}") as monitor:
                ...
                throughput = size / monitor.metrics[-1].execution_time if monitor.metrics else 0
        ...
        for i, result in enumerate(results):
            ...
            if i > 0:
                ...
                assert time_factor < scale_factor * 1.5, \
            ...
            memory_per_record = result['memory_peak'] / size * 1024 * 1024  # bytes per record
            assert memory_per_record < 1000, \
    ...
    def test_concurrent_read_write_performance(self, temp_db, performance_monitor):
        ...
        with performance_monitor.monitor_test("concurrent_read_write") as monitor:
            ...
            def reader_worker(worker_id: int, operations: int) -> Dict[str, Any]:
                ...
                return {
                    ...
                    'p95_time': statistics.quantiles(read_times, n=20)[18] if len(read_times) > 20 else 0
                }
            ...
            def writer_worker(worker_id: int, operations: int) -> Dict[str, Any]:
                ...
                for i in range(operations):
                    try:
                        ...
                        cursor.execute("""
                            ...
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                        """, (
                            (worker_id * operations + i) % 5000 + 1,  # Random work_id
                            "episode",
                            str(i + 1),
                            "テスト配信サイト",
                            "2024-01-15",
                ...
                return {
                    ...
                    'p95_time': statistics.quantiles(write_times, n=20)[18] if len(write_times) > 20 else 0
                }
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=num_readers + num_writers) as executor:
                ...
                # Submit writer tasks
                writer_futures = [
                    executor.submit(writer_worker, i + num_readers, operations_per_worker)
        ...
        assert error_rate < 0.01, f"Error rate {error_rate:.2%} too high for concurrent operations"
        ...
        assert avg_write_time < 0.2, f"Average write time {avg_write_time:.3f}s too slow"
        
        # P95 latency should be reasonable
        p95_read_time = statistics.mean([r['p95_time'] for r in reader_results if r['p95_time'] > 0])
        p95_write_time = statistics.mean([r['p95_time'] for r in writer_results if r['p95_time'] > 0])
        
        if p95_read_time > 0:
            assert p95_read_time < 0.5, f"P95 read time {p95_read_time:.3f}s too slow"
        if p95_write_time > 0:
            assert p95_write_time < 1.0, f"P95 write time {p95_write_time:.3f}s too slow"
...
class TestAdvancedAPIPerformance:
    ...
    async def test_api_load_simulation(self, performance_monitor, benchmark_config):
        ...
        for pattern in load_patterns:
            with performance_monitor.monitor_test(f"api_load_{pattern['name']}") as monitor:
                ...
                async def api_client(client_id: int, num_requests: int):
                    ...
                    with patch('aiohttp.ClientSession') as mock_session_class:
                        ...
                        mock_response.json.return_value = {
                            "data": {"client_id": client_id, "request_count": num_requests}
                        }
                        ...
                        for req_num in range(num_requests):
                            try:
                                ...
                                processing_delay = 0.01 + (req_num % 10) * 0.005  # 10-60ms
                    ...
                    return {
                        ...
                        'p95_response_time': (
                            ...
                            if len(request_times) > 20 else 0
                        ),
                        'p99_response_time': (
                            statistics.quantiles(request_times, n=100)[98] 
                            if len(request_times) > 100 else 0
                        ),
                ...
                p95_response_times = [r['p95_response_time'] for r in client_results if r['p95_response_time'] > 0]
                p99_response_times = [r['p99_response_time'] for r in client_results if r['p99_response_time'] > 0]
                
                execution_time = monitor.metrics[-1].execution_time if monitor.metrics else 0
                ...
                pattern_result = {
                    ...
                    'p99_response_time': statistics.mean(p99_response_times) if p99_response_times else 0,
                    'execution_time': execution_time
                }
        ...
        for result in results:
            ...
            min_throughput = 10 if pattern_name == "light_load" else 20 if pattern_name == "medium_load" else 30
        ...
        response_time_factor = heavy_result['avg_response_time'] / light_result['avg_response_time'] if light_result['avg_response_time'] > 0 else 1
        
        # Response time degradation should be reasonable
        assert response_time_factor < load_factor * 2, \
...
class TestSystemResourceOptimization:
    ...
    def test_memory_optimization_patterns(self, temp_db, performance_monitor):
        ...
        if memory_snapshots:
            ...
            assert memory_growth < 200, f"Memory growth {memory_growth:.1f}MB too high during processing"
            
            # Memory should be relatively stable after GC
            recent_snapshots = memory_snapshots[-5:] if len(memory_snapshots) >= 5 else memory_snapshots
            memory_variance = statistics.variance(recent_snapshots) if len(recent_snapshots) > 1 else 0
            assert memory_variance < 100, f"Memory variance {memory_variance:.1f} too high, poor memory management"
    ...
    def test_cpu_utilization_efficiency(self, temp_db, performance_monitor):
        ...
        assert processing_rate > 1000, f"Processing rate {processing_rate:.0f} records/s too low"
        ...
        if cpu_snapshots:
            ...
            assert avg_cpu_usage < 90, f"Average CPU usage {avg_cpu_usage:.1f}% too high"
...
def performance_report_generator():
    ...
    def generate_report(metrics_list: List[PerformanceMetrics], output_file: str = None):
        ...
        report = {
            ...
            "performance_summary": {
                ...
                "avg_memory_usage": statistics.mean([m.memory_peak_mb for m in metrics_list]),
                "avg_cpu_usage": statistics.mean([m.cpu_usage_percent for m in metrics_list])
            },
...
class TestPerformanceRegression:
    ...
    @pytest.mark.performance
    def test_performance_regression_detection(self, temp_db, performance_monitor, performance_report_generator):
        ...
        with performance_monitor.monitor_test("regression_db_baseline") as monitor:
            ...
            # Standard operations
            test_data = [(f"回帰テスト{i}", f"かいきてすと{i}", f"Regression Test {i}", "anime", f"https://example.com/{i}") 
                        for i in range(1000)]
        ...
        with performance_monitor.monitor_test("regression_db_degraded") as monitor:
            ...
            for i in range(1000):
                ...
                cursor.execute("""
                    INSERT INTO works (title, title_kana, title_en, type, official_url)
                    VALUES (?, ?, ?, ?, ?)
                """, (f"回帰劣化{i}", f"かいきれっか{i}", f"Regression Degraded {i}", "manga", f"https://example.com/degraded/{i}"))
        ...
        regression_report = performance_report_generator(all_metrics, "performance-regression-report.json")
        ...
        assert memory_ratio < memory_regression_threshold, \
