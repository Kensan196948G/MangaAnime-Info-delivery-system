...
class TestGmailAPI:
    ...
    def test_gmail_service_authentication(self, test_config):
        ...
        with patch('google.auth.load_credentials_from_file') as mock_load_creds, \
    ...
    @pytest.mark.integration
    @pytest.mark.api
    def test_send_email_with_multiple_releases(self, mock_gmail_service, sample_work_data, sample_release_data):
    ...
    def test_gmail_api_error_handling(self, mock_gmail_service):
        ...
        mock_gmail_service.users().messages().send().execute.side_effect = error_response
...
class TestCalendarAPI:
    ...
    @pytest.mark.integration
    @pytest.mark.api
    def test_create_calendar_event(self, mock_calendar_service, sample_work_data, sample_release_data):
    ...
    @pytest.mark.integration
    @pytest.mark.api
    def test_create_multiple_calendar_events(self, mock_calendar_service, sample_work_data, sample_release_data):
    ...
    def test_event_datetime_formatting(self):
        ...
        start_datetime = datetime.strptime(f"{release_date} {release_time}", "%Y-%m-%d %H:%M")
...
class TestGoogleAPIsIntegration:
    ...
    @pytest.mark.integration
    @pytest.mark.api
    def test_combined_notification_workflow(self, mock_gmail_service, mock_calendar_service, sample_work_data, sample_release_data):
        ...
        email_message = MIMEText(f'{work["title"]} {release["release_type"]} {release["number"]} 配信開始', 'plain', 'utf-8')
    ...
    def test_batch_notification_performance(self, mock_gmail_service, mock_calendar_service, test_data_generator, performance_test_config):
        ...
        # Mock fast responses
        mock_gmail_service.users().messages().send().execute.return_value = {'id': 'email_id'}
        mock_calendar_service.events().insert().execute.return_value = {'id': 'event_id'}
        ...
        # Verify performance
        max_time = performance_test_config['max_response_time'] * len(anime_data) * 2  # 2 API calls per item
        assert total_time < max_time, f"Batch notifications took {total_time:.2f}s, should be under {max_time:.2f}s"
