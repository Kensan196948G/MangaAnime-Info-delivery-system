...
class TestReleaseNotifier:
    ...
    def test_initialize_components(self):
        ...
        with patch('modules.db.DatabaseManager') as mock_db, \
             patch('modules.mailer.EmailNotifier') as mock_email, \
             patch('modules.calendar.CalendarManager') as mock_calendar:
    ...
    def test_process_new_releases(self):
        ...
        with patch.object(self.notifier, 'db') as mock_db, \
             patch.object(self.notifier, 'email_notifier') as mock_email, \
             patch.object(self.notifier, 'calendar_manager') as mock_calendar:
    ...
    def test_run_daily_check(self):
        ...
        with patch.object(self.notifier, 'collect_anime_data') as mock_anime, \
             patch.object(self.notifier, 'collect_manga_data') as mock_manga, \
             patch.object(self.notifier, 'process_new_releases') as mock_process:
        ...
        valid_configs = [
            {
                ...
                "api": {"enabled": True, "anilist": {"base_url": "https://graphql.anilist.co"}},
...
class TestApplicationIntegration:
    ...
    @patch('modules.anime_anilist.AniListAPI')
    @patch('modules.manga_rss.MangaRSSCollector')
    def test_full_integration_flow(self, mock_manga, mock_anime, mock_calendar, mock_email, mock_db):
    ...
    def test_partial_service_failure(self):
        ...
        with patch.object(ReleaseNotifier, 'collect_anime_data') as mock_anime, \
             patch.object(ReleaseNotifier, 'collect_manga_data') as mock_manga:
    ...
    def test_performance_with_large_dataset(self):
        ...
        large_manga_dataset = [
            {"title": f"Manga {i} Vol. 1", "volume": "1"}
            for i in range(100)
        ]
