...
class TestConfigManager:
    ...
    def test_get_rss_feeds_config(self, sample_config_data):
        ...
        enabled_feeds = [feed for feed in rss_config['enabled_feeds'] if feed.get('enabled', True)]
    ...
    def test_get_enabled_rss_feeds_by_category(self, sample_config_data):
        ...
        manga_feeds = [feed for feed in rss_config['enabled_feeds'] 
                      if feed.get('category') == 'manga' and feed.get('enabled', True)]
        
        anime_feeds = [feed for feed in rss_config['enabled_feeds'] 
    ...
    def test_config_deep_merge(self, sample_config_data):
        ...
        assert merged_config['apis']['anilist']['graphql_url'] == "https://graphql.anilist.co"
    ...
    def test_config_template_generation(self):
        ...
        assert template['notification']['email']['sender_email'] == "your-email@example.com"
    ...
    def _apply_env_overrides(self, config: dict) -> dict:
        ...
        if env_email_enabled:
            result['notification']['email']['enabled'] = env_email_enabled.lower() == 'true'
...
class TestConfigPerformance:
    ...
    def test_config_loading_performance(self, temp_config_file):
        ...
        assert total_time < 1.0, f"Config loading took {total_time:.3f}s for 100 iterations"
    ...
    def test_config_caching_mechanism(self):
        ...
        assert total_time < 0.1, f"Cached config access took {total_time:.3f}s for 1000 iterations"