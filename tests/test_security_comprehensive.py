...
from modules.security_utils import (
    InputSanitizer, RateLimiter, SecureTokenManager, 
    SecureConfig, DatabaseSecurity, SecurityMonitor
)
...
class TestAuthenticationSecurity:
    ...
    def test_token_file_permissions(self, tmp_path):
        ...
        assert permissions == '600', f"Token file permissions are {permissions}, should be 600"
...
class TestDatabaseSecurity:
            ...
            try:
                ...
                if db_security.validate_work_data(malicious_input, 'anime'):
                    ...
                    cursor = db_security.safe_execute(
                        "INSERT INTO test_works (title) VALUES (?)",
                        (malicious_input,)
                    )
                    conn.commit()
                    ...
                    assert count <= len([i for i in malicious_inputs[:malicious_inputs.index(malicious_input)+1]
    ...
    def test_data_validation_before_db_operations(self):
        ...
        # Valid data
        assert db_security.validate_work_data("Valid Title", "anime", "https://example.com") is True
        ...
        assert db_security.validate_work_data("Title", "invalid_type") is False  # Invalid type
        assert db_security.validate_work_data("Title", "anime", "not_a_url") is False  # Invalid URL
...
class TestRateLimitingSecurity:
    ...
    def test_rate_limiter_different_apis(self):
        ...
        assert rate_limiter.is_allowed("api_b", 5) is True  # Different API should still work
...
class TestSecurityMonitoring:
    ...
    def test_security_event_logging(self):
        ...
        monitor.log_security_event('AUTH_FAILURE', {'service': 'gmail', 'reason': 'invalid_token'})
    ...
    def test_rate_limit_violation_detection(self):
        ...
        abuse_events = [e for e in monitor.security_events if e['event_type'] == 'RATE_LIMIT_ABUSE']
    ...
    def test_authentication_failure_monitoring(self):
        ...
        auth_events = [e for e in monitor.security_events if e['event_type'] == 'AUTH_FAILURE']
...
class TestSecurityPerformance:
    ...
    def test_input_sanitization_performance(self):
        ...
        assert processing_time < 1.0, f"Sanitization took {processing_time:.2f}s for 1000 inputs"
    ...
    def test_rate_limiter_performance(self):
        ...
        processing_time = end_time - start_time
        assert processing_time < 1.0, f"Rate limiting took {processing_time:.2f}s for 1000 checks"
    ...
    def test_security_monitoring_performance(self):
        ...
        processing_time = end_time - start_time
        assert processing_time < 0.5, f"Security monitoring took {processing_time:.2f}s for 100 events"
