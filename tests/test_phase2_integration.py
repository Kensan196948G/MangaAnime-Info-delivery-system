    ...
    def test_multiple_rss_feeds_parallel_processing(self, test_config):
        ...
        with patch('requests.Session') as mock_session_class:
            ...
            with ThreadPoolExecutor(max_workers=len(feed_urls)) as executor:
                futures = [
                    executor.submit(mock_session.get, url)
...
class TestDataQualityAndDeduplication:
    ...
    def test_duplicate_detection_accuracy(self, temp_db):
        ...
        test_works = [
            # 完全一致
            {"title": "ワンピース", "title_kana": "わんぴーす", "title_en": "One Piece", "type": "anime"},
            {"title": "ワンピース", "title_kana": "わんぴーす", "title_en": "One Piece", "type": "anime"},
            # 表記ゆれ
            {"title": "鬼滅の刃", "title_kana": "きめつのやいば", "title_en": "Demon Slayer", "type": "anime"},
            {"title": "鬼滅ノ刃", "title_kana": "きめつのやいば", "title_en": "Demon Slayer", "type": "anime"},
            # 異なる作品
            {"title": "呪術廻戦", "title_kana": "じゅじゅつかいせん", "title_en": "Jujutsu Kaisen", "type": "anime"},
        ]
        ...
        for expected in expected_duplicates:
            assert any(expected in title for title in duplicate_titles), f"{expected} の重複が検出されない"
