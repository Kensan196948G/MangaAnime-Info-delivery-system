...
class TestDatabasePerformance:
    ...
    def test_bulk_data_insertion_performance(self, temp_db):
        ...
        assert work_insertion_time < 5.0, f"Work insertion took {work_insertion_time:.2f}s, should be under 5s"
        assert release_insertion_time < 10.0, f"Release insertion took {release_insertion_time:.2f}s, should be under 10s"
    ...
    def test_complex_query_performance(self, temp_db):
        ...
        for i, query in enumerate(complex_queries):
            ...
            assert query_time < 1.0, f"Query {i+1} took {query_time:.3f}s, should be under 1s"
    ...
    def test_concurrent_database_access(self, temp_db):
        ...
        assert overall_ops_per_second > 10, f"Overall throughput {overall_ops_per_second:.1f} ops/s should be > 10"
    ...
    def test_database_memory_usage(self, temp_db):
        ...
        assert max_memory_increase < 100, f"Peak memory increase {max_memory_increase:.1f}MB should be under 100MB"
        assert final_memory_increase < 50, f"Final memory increase {final_memory_increase:.1f}MB should be under 50MB"
        ...
        assert post_query_increase < max_memory_increase + 20, "Query should not significantly increase memory"
        
        conn.close()
    
    def _setup_performance_test_data(self, cursor, work_count: int, releases_per_work: int):
        ...
        # Insert releases
        releases_data = []
        platforms = ["dアニメストア", "Netflix", "Crunchyroll", "BookWalker", "Amazon Prime"]
        
        for work_id in range(1, work_count + 1):
            for release_num in range(1, releases_per_work + 1):
                platform = platforms[work_id % len(platforms)]
                release_date = (datetime.now() - timedelta(days=work_id)).date().isoformat()
    ...
    async def test_anilist_api_rate_limiting_compliance(self):
        ...
        with patch('aiohttp.ClientSession') as mock_session_class:
            ...
            for i in range(test_requests):
                ...
                # Make request
                await mock_session.post("https://graphql.anilist.co", json={"query": "test"})
            ...
            assert requests_per_minute <= max_requests_per_minute, \
                f"Rate limit exceeded: {requests_per_minute:.1f} req/min > {max_requests_per_minute} req/min"
            
            # Verify reasonable performance
            assert total_time < test_requests * 2, f"Total time {total_time:.1f}s too slow for {test_requests} requests"
    ...
    def test_rss_feed_collection_performance(self, mock_rss_feed_data):
        ...
        with patch('requests.get') as mock_get:
            ...
            for feed_url in feed_urls:
                ...
                response = mock_get(feed_url, headers={'User-Agent': 'Test/1.0'}, timeout=20)
            ...
            # Performance assertions
            assert total_collection_time < 10.0, f"RSS collection took {total_collection_time:.2f}s, should be under 10s"
            
            # Individual feed performance
            for result in collection_results:
                assert result['time'] < 3.0, f"Feed {result['url']} took {result['time']:.2f}s, should be under 3s"
                assert result['entries'] > 0, f"Feed {result['url']} returned no entries"
            
            # Average performance
            avg_time_per_feed = total_collection_time / len(feed_urls)
            assert avg_time_per_feed < 2.0, f"Average time per feed {avg_time_per_feed:.2f}s should be under 2s"
    ...
    async def test_concurrent_api_requests_performance(self):
        ...
        async def api_worker(worker_id: int):
            ...
            with patch('aiohttp.ClientSession') as mock_session_class:
                ...
                mock_response = AsyncMock()
                mock_response.status = 200
                mock_response.json.return_value = {"data": {"test": f"worker_{worker_id}"}}
                mock_session.post.return_value.__aenter__.return_value = mock_response
                mock_session_class.return_value.__aenter__.return_value = mock_session
                ...
                for request_num in range(requests_per_connection):
                    ...
                    result = await mock_session.post("https://api.example.com", json={"query": f"test_{request_num}"})
        ...
        # Assertions
        assert total_time < 5.0, f"Concurrent API requests took {total_time:.2f}s, should be under 5s"
        assert overall_throughput > 10, f"Throughput {overall_throughput:.1f} req/s should be > 10"
        ...
        for worker_result in worker_results:
            assert worker_result['total_time'] < 2.0, f"Worker {worker_result['worker_id']} took too long"
...
class TestNotificationPerformance:
    ...
    def test_bulk_email_generation_performance(self, sample_release_data):
        ...
        # Performance assertions
        assert generation_time < 2.0, f"Email generation took {generation_time:.2f}s, should be under 2s"
    ...
    @pytest.mark.performance
    def test_calendar_event_creation_performance(self, mock_calendar_service, sample_release_data):
        ...
        for release in bulk_releases:
            ...
            result = mock_calendar_service.events().insert(
                calendarId='primary',
                body=event_data
            ).execute()
        ...
        # Performance assertions
        assert creation_time < 5.0, f"Calendar creation took {creation_time:.2f}s, should be under 5s"
        assert len(created_events) == 50
        
        # Verify API call efficiency
        events_per_second = len(created_events) / creation_time
        assert events_per_second > 5, f"Event creation rate {events_per_second:.1f}/s should be > 5/s"
    
    @pytest.mark.performance
    @pytest.mark.slow
    def test_full_notification_pipeline_performance(self, temp_db, mock_gmail_service, mock_calendar_service):
        ...
        mock_gmail_service.users().messages().send.return_value.execute.return_value = {
            'id': 'bulk_test_email', 'threadId': 'bulk_thread'
        }
        mock_calendar_service.events().insert.return_value.execute.return_value = {
            'id': 'bulk_test_event', 'status': 'confirmed'
        }
        ...
        for release in unnotified_releases:
            ...
            mock_calendar_service.events().insert(calendarId='primary', body={}).execute()
        ...
        assert notification_time < 20.0, f"Notification time {notification_time:.2f}s should be under 20s"
        assert update_time < 1.0, f"Update time {update_time:.2f}s should be under 1s"
        assert total_pipeline_time < 25.0, f"Total pipeline time {total_pipeline_time:.2f}s should be under 25s"
        
        # Verify throughput
        notifications_per_second = len(sent_notifications) / notification_time
        assert notifications_per_second > 5, f"Notification rate {notifications_per_second:.1f}/s should be > 5/s"
...
class TestSystemResourceUsage:
    ...
    def test_memory_usage_patterns(self, temp_db):
        ...
        max_memory_increase = max(
            ...
            for key in memory_snapshots if key != 'initial'
        )
        
        final_memory_increase = memory_snapshots['after_cleanup'] - initial_memory
        
        # Assertions
        assert max_memory_increase < 200, f"Peak memory increase {max_memory_increase:.1f}MB should be under 200MB"
        assert final_memory_increase < max_memory_increase * 0.5, "Memory should be freed after cleanup"
        
        # Memory growth pattern should be reasonable
        insertion_growth = memory_snapshots['after_insertion'] - initial_memory
        query_growth = memory_snapshots['after_query'] - memory_snapshots['after_insertion']
    ...
    def test_cpu_usage_patterns(self, temp_db):
        ...
        for work in all_works:
            ...
            is_filtered = any(keyword in processed_title.lower() for keyword in ['test', 'cpu'])
        ...
        # Assertions
        assert cpu_time < 5.0, f"CPU intensive processing took {cpu_time:.2f}s, should be under 5s"
        assert processing_rate > 100, f"Processing rate {processing_rate:.1f} items/s should be > 100/s"
    ...
    def test_disk_io_patterns(self, temp_db):
        ...
        avg_size_growth = sum(m['size_growth'] for m in io_measurements) / len(io_measurements)
        total_records = sum(m['records'] for m in io_measurements)
        
        # Assertions
        assert avg_batch_time < 1.0, f"Average batch time {avg_batch_time:.2f}s should be under 1s"
        assert total_growth > 0, "Database should grow with data insertion"
        assert total_growth < 10000, f"Database growth {total_growth:.1f}KB should be reasonable"
        ...
        assert size_growth_variance < avg_size_growth * 2, "Database growth should be relatively consistent"
