    ...
    async def test_anilist_api_client_initialization(self, test_config):
        ...
        with patch("gql.Client") as mock_client_class, patch(
            ...
            transport = AIOHTTPTransport(
                url=config["graphql_url"], timeout=config["timeout_seconds"]
            )
...
class TestAniListPerformance:
    ...
    def test_data_processing_performance(self, test_data_generator):
        ...
        for anime in large_anime_dataset:
            ...
            processed_title = (
                title.get("romaji") or title.get("english") or title.get("native")
            )
    ...
    async def test_memory_usage_monitoring(self, performance_test_config):
        ...
        with patch("gql.Client") as mock_client_class:
            ...
            for i in range(10):
                ...
                processed_media = [
                    {"id": m["id"], "title": m["title"]["romaji"]} for m in media_list
                ]
        ...
        ), f"Memory usage increased by {memory_increase:.2f}MB, limit is {memory_limit}MB"
