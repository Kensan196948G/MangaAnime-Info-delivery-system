...
class TestEmailNotificationIntegration:
    ...
    def test_gmail_send_message(self, mock_gmail_service, sample_release_data):
        ...
        # Prepare message
        message = {
            'raw': 'base64_encoded_message_content'
        }
    ...
    def test_bulk_email_notification(self, mock_gmail_service, sample_release_data):
        ...
        for release in releases:
            ...
            message = {
                'raw': f'message_for_release_{release.get("work_id")}'
            }
            ...
            mock_gmail_service.users().messages().send.return_value.execute.return_value = mock_response
    ...
    def test_email_attachment_support(self, mock_gmail_service):
        ...
        with patch('requests.get') as mock_get:
            ...
            image_attachment.add_header('Content-Disposition', 'attachment', filename='cover.jpg')
    ...
    def test_email_delivery_confirmation(self, mock_gmail_service):
        ...
        message_info = mock_gmail_service.users().messages().get(
            userId='me',
            id=message_id
        ).execute()
    ...
    def test_email_sending_performance(self, mock_gmail_service, performance_test_config):
        """Test email sending performance under load."""
        import time
        import asyncio
        
        max_response_time = performance_test_config['max_response_time']
        concurrent_requests = min(performance_test_config['concurrent_requests'], 5)  # Limit for email
...
class TestEmailTemplateSystem:
    ...
    def _simple_template_render(self, template: str, data: dict) -> str:
        ...
        if 'genres' in data and isinstance(data['genres'], list):
            ...
            result = result.replace('{{#each genres}}\n<li>{{this}}</li>\n{{/each}}', genre_html)
...
class TestEmailNotificationWorkflow:
    ...
    @pytest.mark.integration
    @pytest.mark.e2e
    def test_end_to_end_notification_workflow(self, mock_gmail_service, temp_db, 
                                              sample_work_data, sample_release_data):
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (work_id, release['release_type'], release['number'], 
              release['platform'], release['release_date'], release['source'], 
              release['source_url'], 0))  # Not notified yet
        ...
        for release_row in unnotified_releases:
            ...
            result = mock_gmail_service.users().messages().send(
                userId='me',
                body={'raw': 'encoded_email_content'}
            ).execute()
