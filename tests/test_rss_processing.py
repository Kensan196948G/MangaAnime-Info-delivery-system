...
class TestRSSFeedProcessing:
    ...
    def test_rss_date_parsing(self, mock_rss_feed_data):
        ...
        for entry in parsed_feed.entries:
            ...
            if hasattr(entry, 'published'):
                ...
                now = datetime.now()
                assert published_date < now + timedelta(days=365)  # Not more than 1 year in future
                assert published_date > now - timedelta(days=365*5)  # Not more than 5 years in past
    ...
    def test_rss_feed_fetching_mock(self, mock_requests_session, test_config):
        ...
        with patch('requests.Session', return_value=mock_requests_session):
            ...
            response = session.get(feed_url, timeout=test_config['apis']['rss_feeds']['timeout_seconds'])
            ...
            for pattern in title_patterns:
                ...
                if match:
                    ...
                    if len(match.groups()) >= 2:
                        episode_number = match.group(2) if match.group(2) != '最終話' else 'final'
    ...
    def test_concurrent_rss_processing(self, performance_test_config):
        ...
        async def test_concurrent_processing():
            ...
            async with aiohttp.ClientSession() as session:
                ...
                max_sequential_time = len(feeds) * 0.2  # Each feed takes ~0.1s + overhead
        ...
        with patch('requests.Session') as mock_session_class:
            ...
            def mock_get_with_recovery(url, **kwargs):
                ...
                if call_count <= 2:  # Fail first 2 attempts
                    ...
                    response = Mock()
                    response.status_code = 200
                    response.text = "<rss><channel><title>Recovery Test</title></channel></rss>"
                    return response
