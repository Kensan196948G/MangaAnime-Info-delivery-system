...
class TestCalendarIntegration:
    ...
    @pytest.mark.integration
    def test_bulk_calendar_event_creation(self, mock_calendar_service, sample_release_data):
    ...
    @pytest.mark.integration
    def test_calendar_event_with_reminders(self, mock_calendar_service, sample_release_data):
        ...
        assert any(r['method'] == 'popup' and r['minutes'] == 30 for r in result['reminders']['overrides'])
        assert any(r['method'] == 'email' and r['minutes'] == 1440 for r in result['reminders']['overrides'])
    ...
    def test_calendar_event_conflict_detection(self, mock_calendar_service):
        ...
        mock_calendar_service.events().list.return_value.execute.return_value = existing_events
        ...
        existing_event_summaries = [event['summary'] for event in events_result['items']]
        
        # Test conflict detection
        new_event_summary = 'ワンピース - エピソード 1050'
        
        has_conflict = any(new_event_summary in existing_summary 
        ...
        has_conflict_unique = any(unique_event_summary in existing_summary 
                                 for existing_summary in existing_event_summaries)
    ...
    def test_calendar_event_update_and_deletion(self, mock_calendar_service):
        ...
        mock_calendar_service.events().get.return_value.execute.return_value = original_event
        
        # Get existing event
        event = mock_calendar_service.events().get(
            calendarId='primary',
            eventId=event_id
        ).execute()
        ...
        mock_calendar_service.events().update.return_value.execute.return_value = updated_event
        
        # Perform update
        result = mock_calendar_service.events().update(
            calendarId='primary',
            eventId=event_id,
            body=updated_event
        ).execute()
        ...
        delete_result = mock_calendar_service.events().delete(
            calendarId='primary',
            eventId=event_id
        ).execute()
    ...
    def test_calendar_multiple_calendar_support(self, mock_calendar_service):
        ...
        for category, calendar_id in calendars.items():
            event_data = {
                ...
                'start': {'date': '2024-01-15'},
                'end': {'date': '2024-01-15'},
                'colorId': '3' if category == 'anime' else '6'  # Blue for anime, orange for manga
            }
    ...
    def test_calendar_timezone_handling(self, mock_calendar_service):
        ...
        for timezone in timezones:
            event_data = {
                ...
                'start': {
                    'dateTime': f'{base_time}+09:00' if timezone == 'Asia/Tokyo' else f'{base_time}Z',
                    'timeZone': timezone
                },
                'end': {
                    'dateTime': f'{base_time}+09:00' if timezone == 'Asia/Tokyo' else f'{base_time}Z',
    ...
    def test_calendar_batch_operations_performance(self, mock_calendar_service, performance_test_config):
...
詳細: {release.get('source_url', 'なし')}"""
    ...
    def test_manga_event_formatting(self, sample_work_data, sample_release_data):
        ...
        event_description = self._format_manga_event_description(manga_work, volume_release)
    ...
    def test_event_duration_calculation(self):
        ...
        for content_type, expected_duration in duration_map.items():
            ...
            if expected_duration == 0:
                ...
                event_start = {'dateTime': start_time.isoformat() + '+09:00', 'timeZone': 'Asia/Tokyo'}
                event_end = {'dateTime': end_time.isoformat() + '+09:00', 'timeZone': 'Asia/Tokyo'}
            
            # Verify duration calculation
            if expected_duration == 0:
                assert 'date' in event_start and 'date' in event_end
            else:
                start_dt = datetime.fromisoformat(event_start['dateTime'].replace('+09:00', ''))
                end_dt = datetime.fromisoformat(event_end['dateTime'].replace('+09:00', ''))
