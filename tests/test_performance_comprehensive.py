...
class TestDatabasePerformance:
    ...
    def test_database_bulk_insert_performance(self, temp_db, performance_suite):
        ...
        for size in test_sizes:
            with performance_suite.measure_operation(f"bulk_insert_{size}"):
                # Generate test data
                works_data = [
                    (f"Test Work {i}", f"てすとわーく{i}", f"Test Work {i} EN", "anime", f"https://test{i}.com")
                    for i in range(size)
                ]
        ...
        assert summary['max_response_time_ms'] < 5000, f"Bulk insert too slow: {summary['max_response_time_ms']}ms"
        assert summary['avg_memory_usage_mb'] < 200, f"Memory usage too high: {summary['avg_memory_usage_mb']}MB"
    ...
    def test_database_query_performance(self, temp_db, performance_suite):
        ...
        for i in range(1000):
            ...
            db_manager.insert_release(i+1, "episode", str(i+1), "test", "2024-01-15", "test", "https://test.com", 0)
        ...
        summary = performance_suite.get_metrics_summary()
        assert summary['avg_response_time_ms'] < 100, f"Query too slow: {summary['avg_response_time_ms']}ms"
    ...
    def test_database_concurrent_access(self, temp_db, performance_suite):
        ...
        def concurrent_operation(thread_id: int):
            with performance_suite.measure_operation(f"concurrent_db_{thread_id}"):
                for i in range(10):
                    db_manager.insert_work(f"Concurrent Work {thread_id}_{i}", None, None, "anime", None)
...
class TestAPIPerformance:
    ...
    def test_anilist_api_performance(self, mock_client, performance_suite):
        ...
        client = AniListClient({'graphql_url': 'https://test.com', 'rate_limit': {'requests_per_minute': 90}})
        ...
        summary = performance_suite.get_metrics_summary()
        assert summary['avg_response_time_ms'] < 1000, f"API calls too slow: {summary['avg_response_time_ms']}ms"
    ...
    def test_rss_processing_performance(self, mock_processor, performance_suite):
        ...
        summary = performance_suite.get_metrics_summary()
        assert summary['max_response_time_ms'] < 2000, f"RSS processing too slow: {summary['max_response_time_ms']}ms"
...
class TestFilteringPerformance:
    ...
    def test_large_dataset_filtering(self, performance_suite):
        ...
        with performance_suite.measure_operation("filter_large_anime_dataset"):
            filtered_anime = filter_logic.filter_anime_batch(large_dataset['anime_data'])
        ...
        with performance_suite.measure_operation("filter_large_manga_dataset"):
            filtered_manga = filter_logic.filter_manga_batch(large_dataset['manga_data'])
            assert len(filtered_manga) <= len(large_dataset['manga_data'])
        
        summary = performance_suite.get_metrics_summary()
        assert summary['avg_response_time_ms'] < 500, f"Filtering too slow: {summary['avg_response_time_ms']}ms"
    ...
    def test_regex_vs_string_matching_performance(self, performance_suite):
        ...
        assert all(m.response_time_ms < 1000 for m in performance_suite.metrics), "Matching methods too slow"
...
class TestEndToEndPerformance:
    ...
    def test_full_workflow_performance(self, mock_calendar, mock_mailer, mock_rss, mock_anilist, 
                                     temp_db, performance_suite):
        """Test complete workflow performance"""
        # Setup mocks
        mock_services = create_mock_services()
        
        mock_anilist.return_value.fetch_current_season_anime = AsyncMock(
            return_value=mock_services['data_factory'].create_large_dataset(100)['anime_data']
        )
        mock_rss.return_value.process_all_feeds = AsyncMock(
            return_value=mock_services['data_factory'].create_large_dataset(50)['manga_data']
        )
        mock_mailer.return_value.send_notification = Mock(return_value=True)
        mock_calendar.return_value.create_event = Mock(return_value={'id': 'test_event'})
        ...
        with performance_suite.measure_operation("full_workflow"):
            ...
            for i in range(notification_count):
                mock_mailer.return_value.send_notification(f"test{i}@example.com", "Test", "Content")
                mock_calendar.return_value.create_event(f"Event {i}", "2024-01-15")
        
        summary = performance_suite.get_metrics_summary()
        
        # Full workflow should complete within reasonable time
        assert summary['max_response_time_ms'] < 30000, f"Full workflow too slow: {summary['max_response_time_ms']}ms"
        assert summary['max_memory_usage_mb'] < 500, f"Memory usage too high: {summary['max_memory_usage_mb']}MB"
    ...
    def test_system_under_load(self, performance_suite):
        ...
        for load in load_levels:
            ...
            with performance_suite.measure_operation(f"system_load_{load}"):
                ...
                assert response_time < 5.0, f"Response time too high under load {load}: {response_time}s"
                assert memory_usage < 1000, f"Memory usage too high under load {load}: {memory_usage}MB"
                assert cpu_usage < 95, f"CPU usage too high under load {load}: {cpu_usage}%"
...
class TestMemoryLeakDetection:
    ...
    def test_database_connection_cleanup(self, temp_db, performance_suite):
        ...
        assert memory_growth < 20, f"Database connection leak detected: {memory_growth}MB growth"


# Utility function for performance test reporting
def generate_performance_report(performance_suite: PerformanceTestSuite, output_file: str = None):
