...
// =============================================================================
async function sendTestNotificationGlobal() {
    try {
        showNotification('テスト通知を送信しています...', 'info', 2000);
        ...
        if (response.ok && data.success) {
            const successMsg = data.message || 'テスト通知を送信しました。メールボックスを確認してください。';
            showNotification(successMsg, 'success', 5000);
            console.log('Test notification sent successfully:', data);
            return { success: true, data };
        } else {
            const errorMsg = data.error || `HTTP ${response.status}: ${response.statusText}`;
            const fullErrorMsg = response.status === 400 
                ? '設定エラー: ' + errorMsg 
                : response.status === 500 
                ? 'サーバーエラー: ' + errorMsg 
                : errorMsg;
            
            showNotification(fullErrorMsg, 'error', 8000);
            console.error('Test notification failed:', data);
            return { success: false, error: fullErrorMsg };
        }
    } catch (error) {
        const errorMsg = error.message || 'テスト通知の送信に失敗しました';
        const fullErrorMsg = error instanceof TypeError && error.message.includes('fetch')
...
if (window.AnimeManagaSystem) {
    window.AnimeManagaSystem.sendTestNotification = sendTestNotificationGlobal;
}
