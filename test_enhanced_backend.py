            ...
            try:
                with db.get_connection() as conn:
                    ...
                    work_id = db.create_work(
                        title=f"Test Work {operation_id}",
                        work_type="anime"
                    )
                    ...
                    logger.info(f"Operation {operation_id}: Created work {work_id}, release {release_id}")
                    return True
            except Exception as e:
...
def test_error_recovery():
    ...
    try:
        from enhanced_error_recovery import (
            EnhancedErrorRecovery, ErrorSeverity, 
            initialize_error_recovery, record_error, record_success
        )
        ...
        record_error("anilist_api", "rate_limit_error", "API rate limit exceeded", ErrorSeverity.MEDIUM)
        record_error("manga_rss", "timeout_error", "RSS feed timeout", ErrorSeverity.LOW)
        record_error("database", "connection_error", "Database connection failed", ErrorSeverity.HIGH)
        ...
        for error in recent_errors:
            logger.info(f"  {error['component']}: {error['error_type']} - {error['message']}")
