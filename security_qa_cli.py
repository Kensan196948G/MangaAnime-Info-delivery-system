...
def run_security_audit(project_path: str, output_file: Optional[str] = None, verbose: bool = False) -> Dict[str, Any]:
...
def run_qa_audit(project_path: str, db_path: Optional[str] = None, output_file: Optional[str] = None, verbose: bool = False) -> Dict[str, Any]:
    ...
    print(f"Quality Level: {qa_results.get('summary', {}).get('quality_level', 'UNKNOWN')}")
...
def run_combined_audit(project_path: str, db_path: Optional[str] = None, output_dir: Optional[str] = None, verbose: bool = False) -> Dict[str, Any]:
    ...
    total_high = security_results.get('high_findings', 0) + qa_results.get('summary', {}).get('total_issues_found', 0)
    ...
    if all_recommendations:
        ...
        if len(all_recommendations) > 5:
            print(f"  ... and {len(all_recommendations) - 5} more (see detailed reports)")
    ...
    if output_dir:
        ...
        combined_report = {
            ...
            'summary': {
                ...
                'health_status': health_status.replace('🟢 ', '').replace('🔵 ', '').replace('🟡 ', '').replace('🟠 ', '').replace('🔴 ', ''),
...
Examples:
  ...
  %(prog)s --project /path/to/project         # Specify project path
    ...
    parser.add_argument(
        '--database', '-d',
        help='Database path (default: PROJECT/db.sqlite3)'
    )
    ...
    try:
        ...
        if args.mode == 'security':
            results = run_security_audit(
                str(project_path),
                str(Path(args.output) / 'security_report.json') if args.output else None,
                args.verbose
            )
        ...
        if args.mode == 'full':
            ...
            score = results.get('overall_score', results.get('security_score', results.get('qa_score', 0)))
            if score < 60:
                sys.exit(2)
            elif score < 80:
                sys.exit(1)
