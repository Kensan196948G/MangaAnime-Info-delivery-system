...
def check_test_status():
    ...
    try:
        result = subprocess.run([
            sys.executable, "-c", "import pytest; print(f'pytest {pytest.__version__}')"
    ...
    print(f"  - Test files: {len(list(tests_dir.glob('*.py'))) if tests_dir.exists() else 0}")
    print(f"  - Ready to run: {'✅' if tests_dir.exists() and any(tests_dir.glob('*.py')) else '❌'}")
